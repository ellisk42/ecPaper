...
\begin{tikzpicture}[scale=2,line width=1.5mm]

  \node[latent,scale=2,align=center] at (3.5,3) (dx){DSL};
  \node[ellipse,fill=white,draw=black,inner sep=1pt,
minimum size=20pt, font=\fontsize{10}{10}\selectfont, node distance=1,scale=2] at ([xshift=2cm]dx) (zp){program};
  \node[obs,scale=2] at ([xshift=2cm]zp) (xp) {task};
  \plate {}{(zp)(xp)}{};
...
\begin{frame}{Turtle graphics}
  ...
  \begin{block}{DSL}
    \texttt{FW x | RT x | UP | DOWN | SET state}
  \end{block}
