...
DSLs constrain the search
over programs with strong prior knowledge in the form of a restricted
library of primitives tuned to the domain: for
...
\\\noindent\textbf{Wake Cycle}: Given a set of \textbf{tasks}, typically
several hundred, we search for compact programs that solve these tasks.
Currently we take the simple strategy of
...
\\\noindent\textbf{Sleep-R Cycle}: This trains a neural net (the \textbf{R}ecognition model) to
...
programs that have high prior probability according to the current DSL,
