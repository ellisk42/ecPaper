...
\Large
  \begin{tabular}{c}
          ...
          \visible<3->{
          \code{$f(\ell) = $}\code{($f_1$ $\ell$ ($\lambda$ (x) (> x 2)))}}
  \end{tabular}
  \vspace{2cm}
  
  \visible<2->{
    \textbf{Library:}\\
          $f_1(\ell,$\code{p}$) \,=\, $\code{(foldr $\ell$ nil ($\lambda$ (x a) (if (p x) (cons x a) a)))}\\
    \hspace{\helpSize}($f_1$: \emph{Higher-order filter function})\\
    \hspace{\helpSize}(\emph{Get elements} $x$ \emph{ from $\ell$ where \code{(p $x$)} returns true})
    }

\end{frame}

\begin{frame}{Subset of 38 learned library routines for list processing}
  \Large\centering
  \begin{tabular}{ll}
    \begin{tabular}{l}
            $f_0(\ell,$\code{r}$) \,=\, $\code{(foldr r $\ell$ cons)}
      \end{tabular}&$f_0$: \emph{Append lists }\code{r}\emph{ and  $\ell$}\\\\
    \begin{tabular}{l}
      $f_1(\ell,$\code{p}$) \,=\, $\code{(foldr $\ell$ nil ($\lambda$ (x a)}\\
  \hspace{0.5cm}\code{(if (p x) (cons x a) a)))}
      \end{tabular}&    
$f_1$: \emph{Higher-order filter function}\\\\
    \begin{tabular}{l}
      $f_2(\ell) \,=\, $\code{(foldr $\ell$ 0 ($\lambda$ (x a)}\\
\phantom{$f_2(\ell) \,=\, $}\code{(if (> a x) a x)))}
      \end{tabular}
&$f_2$: \emph{Maximum element in list $\ell$}\\\\
    \begin{tabular}{l}
      $f_3(\ell,$\code{k}$) \,=\, $\code{(foldr $\ell$ (is-nil $\ell$)}\\
      \phantom{$f_1(\ell,$}
\code{($\lambda$ (x a) (if a a (= k x))))}
      \end{tabular}
&$f_2$: \emph{Whether $\ell$ contains }\code{k}
    \end{tabular}
  \end{frame}

\begin{frame}{Explore/Compress/Compile as Bayesian Inference}
...
\centering
  ...
  \end{tikzpicture}
\end{frame}
\begin{frame}{Explore/Compress/Compile as {\color{red}Amortized} Bayesian Inference}\centering
...
\begin{frame}{\underline{\textbf{Explore}}/Compress/Compile as {\color{red}Amortized} Bayesian Inference}\centering
  \underline{\textbf{Explore}: Infer programs, fixing DSL and neural recognition model}\vspace{1cm}
...
\begin{tikzpicture}[scale=1,line width=0.5mm]
  ...
  \node[obs,scale=1] at (3.5,3) (dx){DSL};
  \node[latent,scale=1] at ([yshift=-1.5cm,xshift=2cm]dx) (zp){prog};
  \node[obs,scale=1] at ([yshift=-1.45cm]zp) (xp) {task};
  \node[latent,scale=1] at ([xshift=2cm]zp) (zp1){prog};
  \node[obs,scale=1] at ([xshift=2cm]xp) (xp1) {task};
...
\begin{frame}{\underline{\textbf{Explore}}/Compress/Compile as {\color{red}Amortized} Bayesian Inference}\centering
  \underline{\textbf{Explore}: Infer programs, fixing DSL and neural recognition model}%

  
%  \vspace{1cm}

    \begin{tikzpicture}[scale=1.2,line width=0.5mm]
    ...
    \node[align=center] at ([yshift = -2cm]d) (t){\textbf{Task}\\
      ...
      \footnotesize    \code{[1\, 2\, 3\, 4]}$\to$\code{[3\, 4]} \\
      \footnotesize    \code{[4\, 3\, 2\, 1]}$\to$\code{[4\, 3]} };

    \node at ([xshift = 1.3cm]t.east) (nn){
    ...
    \node[align = center, text width = 1cm] at ([yshift = 0.5cm,xshift=0cm]nn.north) {\baselineskip=0pt \small Recog. model\par};
...
\small    \code{($f_1$ $\ell$ ($\lambda$ (x) (> x 2)))}};
    ...
    \draw [->  ,rounded corners] (t.south) -- ([yshift = -0.5cm]t.south) -- ([yshift = -0.5cm] s.south |- t.south) -- (s.south);
      ...
      \node at ([yshift=-2.8cm,xshift=-0.5cm]f.south) {
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      \tikzstyle{neuron}=[circle,fill=blue!50,minimum size=20pt]
      \fill[fill=white] (-0.25,-0.5) rectangle (2.25,-4.5);
      \node[rectangle] at (1,1) {};
      ...
      \foreach \source in {1,...,3}
          \foreach \dest in {1,...,4}
              \draw [-latex] (H-\source) -- (O-\dest);
    \end{tikzpicture}}
\begin{tikzpicture}[scale=0.8,line width=0.25mm]
  ...
  \draw [->,red] ([xshift=2cm]dx.east) -- ([xshift=3cm]dx.east);
  \node at ([xshift=3.2cm]dx.east]) (uses){is};k
  \node at ([xshift=0.5cm]uses.east) (nnn){\NeuralNetwork{0.12}};
  \draw[thin] ([xshift=1.9cm,yshift=0.4cm]dx.east) rectangle
  ([xshift=0.5cm,yshift=-0.3cm]nnn.south);
  \end{tikzpicture}
      };
...
\begin{frame}{Explore/\underline{\textbf{Compress}}/Compile as {\color{red}Amortized} Bayesian Inference}\centering
  \underline{\textbf{Compress}: Update DSL, fixing programs}
\vspace{1cm}
...
\begin{tikzpicture}[scale=1,line width=0.5mm]
  ...
  \node[latent,scale=1] at (3.5,3) (dx){DSL};
  \node[obs,scale=1] at ([yshift=-1.5cm,xshift=2cm]dx) (zp){prog};
  \node[obs,scale=1] at ([yshift=-1.45cm]zp) (xp) {task};
...
\begin{frame}{Explore/\underline{\textbf{Compress}}/Compile as {\color{red}Amortized} Bayesian Inference}\centering
  ...
  \begin{tikzpicture}[scale=0.4,line width=0.5mm]
    ...
    \node(c)[rectangle, rounded corners, draw, minimum width = 8cm, minimum height = 3cm, anchor = north west] at (5.2,1) {};
...
\end{tikzpicture}
    ...
    \node at ([yshift=-5.8cm,xshift=-0.5cm]d.south) {
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      \tikzstyle{neuron}=[circle,fill=blue!50,minimum size=20pt]
      \fill[fill=white] (-0.25,-0.5) rectangle (2.25,-4.5);
      \node[rectangle] at (1,1) {};
...
\begin{tikzpicture}[scale=0.8,line width=0.25mm]
  ...
  \draw [->,red] ([xshift=2cm]dx.east) -- ([xshift=3cm]dx.east);
  \node at ([xshift=3.2cm]dx.east]) (uses){is};
  \node at ([xshift=0.5cm]uses.east) (nnn){\NeuralNetwork{0.12}};
  \draw[thin] ([xshift=1.9cm,yshift=0.4cm]dx.east) rectangle
  ([xshift=0.5cm,yshift=-0.3cm]nnn.south);
  \end{tikzpicture}
...
\end{frame}
\begin{frame}{Explore/Compress/\underline{\textbf{Compile}} as {\color{red}Amortized} Bayesian Inference}\centering
\underline{\textbf{Compile}: Train {\color{red} recognition model}}
\vspace{1cm}
  ...
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      ...
      \foreach \source in {1,...,3}
          \foreach \dest in {1,...,4}
              \draw [-latex] (H-\source) -- (O-\dest);
    \end{tikzpicture}}
\hspace{1.5cm}\begin{tikzpicture}[scale=1.2,line width=0.5mm]
            \node[align=center] at (-3,-1) (d){\Large DSL};
      %    \node at ([xshift = 3cm]d.east) (p2){program};
      \node at ([xshift=2.5cm,yshift = 0cm]d.east) (p1){\Large  program};

      \draw[squiggle,-> ] (d.east) -- node[sloped,above]{sample} (p1.west);

      \node at ([xshift = 2.5cm]p1.east) (t1){ \Large  task};
    ...
    \node at ([yshift=-3cm,xshift=2cm]t1.south) {
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      \tikzstyle{neuron}=[circle,fill=blue!50,minimum size=20pt]
      \fill[fill=white] (-0.25,-0.5) rectangle (2.25,-4.5);
      \node[rectangle] at (1,1) {};
  ...
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      ...
      \foreach \source in {1,...,3}
          \foreach \dest in {1,...,4}
              \draw [-latex] (H-\source) -- (O-\dest);
    \end{tikzpicture}}
\begin{tikzpicture}[scale=0.7,line width=0.25mm]
  ...
  \end{tikzpicture}
    ...
    \node at ([xshift=5cm,yshift=-2cm]e.south) (cc){
  \newcommand{\NeuralNetwork}[1]{    \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=#1,shift={+(-1,2.5)}}]
      \tikzstyle{neuron}=[circle,fill=blue!50,minimum size=20pt]
      \fill[fill=white] (-0.25,-0.5) rectangle (2.25,-4.5);
      \node[rectangle] at (1,1) {};
...
\begin{tikzpicture}[scale=0.8,line width=0.25mm]
  ...
  \end{tikzpicture}
      ...
      \node at ([xshift=-5cm,yshift=-2cm]e.south) (c){
\begin{tikzpicture}[scale=0.8,line width=0.25mm]
  ...
  \end{tikzpicture}
      ...
      \node at (e.north) {\textbf{\underline{Explore}}};
      \node at ([yshift=-0.25cm]cc.south) {\textbf{\underline{Compile}}};
      \node at ([yshift=-0.25cm]c.south) {\textbf{\underline{Compress}}};
      \draw[->] (c.east) -- (cc.west);
      \draw[->] (e.west) -- (c.north);
      \draw[->] (cc.north) -- (e.east);
    \end{tikzpicture}
  \end{frame}
...
\centering        \includegraphics[width=0.6\textwidth]{figures/textPrimitives.png}
\\      Learned DSL primitives over 3 iterations (3 columns). Learned primitives call each other (arrows).

  \end{frame}
\begin{frame}{Programs with numerical parameters: Symbolic regression from visual input}
Fits parameters by autograd-ing thru program
\\
\\
\textbf{Recognition model looks at picture of function's graph}
...
\centering  Training tasks:

\includegraphics[width = 0.4\textwidth]{figures/logo90.png}
\end{frame}
\begin{frame}{Generative graphics programs: Samples from DSL}\centering
  \begin{tabular}{c|c}
     DSL samples before learning&DSL samples after learning\\
          \raisebox{-.5\height}{\includegraphics[width = 0.45\textwidth]{figures/initialDreams/montage.png}}&
  \raisebox{-.5\height}{\includegraphics[width = 0.45\textwidth]{figures/finalDreams/montage.png}}
  \end{tabular}
  \end{frame}
