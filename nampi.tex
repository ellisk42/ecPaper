...
programming languages and AI have built successful program induction
algorithms for many applications, such as handwriting recognition and
...
systems such as FlashFill that aim to induce a wide range of programs
...
set of programming primitives tuned to the needs of the domain: for
...
focused on the kinds of problems that people solve relatively
...
using a bidirectional GRU~\cite{cho2014learning} for
...
function \code{filter} (Tbl.~\ref{initialExampleDSL}, left).%% , yielding a more expressive DSL more closely
...
Here we will instead start out with generic sequence manipulation
...
and so the recognition model can  look at a graph and predict
...
\begin{figure}\vspace{-0.0cm} \newcommand{\functionSize}{1.1cm}\centering
  ...
  \begin{minipage}[c]{0.69\columnwidth}    
    \caption{Recognition model input for symbolic regression. DSL learns subroutines for polynomials (top rows) and rational functions (bottom) while the recognition  model jointly learns to look at a graph of the function (left) and predict which learned subroutines best explain the observation.}\label{functions}
        \end{minipage}
\end{figure}
We learn a DSL containing 13 new functions,
mainly templates for polynomials of different orders or ratios of polynomials.
It also learns to find programs minimizing the number of continuous parameters ---
...
How far can we in this direction?
...
and simple arithmetic operations like 
...
measuring how many
tasks are solved and how long it takes to solve them~\ref{learningCurves}.
...
we converge upon DSLs and recognition models that
more closely match the domains.
...
\begin{figure*}\centering
  ...
  (\teal{in teal}) the recognition model. Solid lines: \% holdout testing tasks solved w/ 10m timeout. Dashed lines: Average solve time, averaged only over tasks that are solved.}\label{learningCurves}
\end{figure*}
...
--- can help make program induction systems more generally useful for AI. Many
