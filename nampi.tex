...
\tiny
  \begin{tabular}{ccc}
\multicolumn{3}{c}{Tasks and synthesized programs:}\\
cut&F &Moss Side \\ 
control&CL &Burnage \\
  control&F & City Centre\\
cut& PCFL& Brooklands \\
    \hline
\scode{\textbackslash l*\textbackslash l} & \scode{((\textbackslash u\textbackslash u)*)\textbar F}& \scode{( \textbar\textbackslash u\textbackslash w*)*} \\
    \bottomrule
  \end{tabular}
  \quad
    \begin{tabular}{l}
...
\hline
$f_4(x) = $ \scode{($x$*$x$)} (equivalent to regex `plus')\\
$f_5() = f_4(\scode{\textbackslash l}) = \scode{\textbackslash l*\textbackslash l}$\\
    \bottomrule
  \end{tabular}
\\
\begin{tabular}{ccc}
\multicolumn{3}{c}{samples from synthesized programs:}\\
 \scode{\textbackslash l*\textbackslash l} & \scode{((\textbackslash u\textbackslash u)*)\textbar F}& \scode{( \textbar\textbackslash u\textbackslash w*)*} \\
\hline
ya&DQDF & Vr DR \\ 
fcbadcvoaobvjf&F &BeF lKQ \\
mgs&F& W\\
piljnl& KI& kqBfZ 0 \\
    \bottomrule
  \end{tabular}
            \centering 
            \includegraphics[width=.25\textwidth]{figures/nampiregexjoshcurve.png}   
\caption{regex stuff}
\end{figure}
