...
\section{Introduction: The Centrality of Domain-Specific Languages}
...
We evaluated \system on random 50/50 test/train split.  The system
...
matching the domain (left of Tbl.~\ref{initialExampleDSL}, right of
...
``Learning from scratch'' is subjective, but we believe a reasonable
starting point is the set of primitives provided in McCarthy's 1959
Lisp implementation~\cite{mccarthy1960recursive}: these include
conditionals, \code{lambda}'s, recursion, arithmetic, and the basic
Lisp operators \code{cons}, \code{car}, \code{cdr}, and \code{nil}.
