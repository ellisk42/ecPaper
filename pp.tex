...
\Wider[5.4em]{
  \begin{tikzpicture}
    ...
    \visible<2->{
      ...
      \node(tower) at ([xshift=1.5cm]Lisp.east) {\includegraphics[width = 3cm]{figures/pyramid1.png}};
...
\begin{frame}{DreamCoder --- Sleep-G (\textbf{Refactoring})}
  ...
  \begin{tabular}{cc}
    \toprule Task&Program\\\midrule
    \begin{tabular}{l}
      \texttt{(1 2)}$\to$\texttt{(2 4)}\\
      \texttt{(1 9)}$\to$\texttt{(2 18)}\\
    \end{tabular}&
\begin{tabular}{l}
...
\texttt{ ($\lambda$ (z) (+ z z)))}
    ...
    \end{tabular}\\\midrule
    \begin{tabular}{l}
      \texttt{(1 2)}$\to$\texttt{(2 3)}\\
      \texttt{(1 9)}$\to$\texttt{(2 10)}\\
    \end{tabular}&
    \begin{tabular}{l}
\begin{tabular}{l}
\texttt{(}\orange{\texttt{($\lambda$ (f) (Y ($\lambda$ (r l) (if (nil? l) nil}}\\
