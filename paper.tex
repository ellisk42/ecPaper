...
and it can be used to solve other related program learning problems.
...
while a sleep cycle means updating the DSL and the recognition model
to more closely match the distribution of tasks.
...
Our strategy is to iterate through three steps: (1) a \emph{wake} phase where we search for programs, and then a pair of \emph{sleep} phases: (2) learning a neural recognition model -- which we use
...
\noindent \textbf{Searching for programs:}  Our program search  is informed by both the DSL and the recognition model.
When these improve we find better programs.

\noindent\textbf{Learning a recognition model:} The recognition model is trained both on samples from the DSL and on programs found by the search procedure. As the DSL improves and as search finds more programs, the recognition model gets both more data to train on and better data.

\noindent\textbf{Improving the DSL:} We induce a DSL from the programs we have found so far which solve the tasks;
...
\begin{figure}\centering
  \begin{tikzpicture}
    ...
    \draw [->,thick] (synthesis.-120) to[out = -150,in = 60] node[below,rotate = 45,align = center]{{\footnotesize Trains}\\{\footnotesize (Sleep)}} (DSL.30);
    \draw [->,thick] (synthesis.-60) to[out = -30,in = 120] node[below,rotate=-45,align = center]{{\footnotesize Trains}\\{\footnotesize (Sleep)}} (recognitionModel.150);
    \draw [->,thick] (DSL.east) to[out = -30,in = 210] node[above, align = center]{{\footnotesize Trains}\\{\footnotesize (Sleep)}} (recognitionModel.west);

    \draw [->,thick,dashed] (DSL.north) to[out = 90,in = 180] node[fill=white,align = center]{  \footnotesize{Provides}\\ \footnotesize{inductive bias}} (synthesis.west);
...
\subsection{Sleep Phase I: Learning a Recognition Model}\label{recognitionSection}
...
\subsection{Sleep Phase II: Inducing the DSL}\label{grammarInductionSection}
...
a lesioned version without the recognition model eventually comes close to catching up (red curves in Figure~\ref{regressionLearningCurve}; No NN in Table~\ref{regressionComparison}).
We compare our model on held-out tasks against 4 baselines (Table~\ref{regressionComparison}):
No NN, which lesions $q$;
RF/DC,
which holds the generative model $(\mathcal{D},\theta)$ fixed and learns
a $q$ only from samples from the fixed generative model --
this is how RobustFill and DeepCoder are trained;
PCFG, which lesions $q$, fixes $\mathcal{D}$, and learns only $\theta$ (so it is analogous to learning the parameters of a PCFG);
and Enum, which does no learning and just enumerates a large frontier.
\begin{figure}\centering
  \includegraphics[width = 7cm]{figures/regression.eps}
  \caption{Learning curves for symbolic regression. Green: \system. Red: no neural network. Notice how the model without the neural network eventually almost catches up.}\label{regressionLearningCurve}
\end{figure}

\begin{table}
\begin{tabular}{lllll}
  \toprule \system&No NN  & RF/DC & PCFG & Enum\\\midrule
  98\% solved&94\%&0.7\%&0\%&2\% \\
  $2.7$ sec& $2.8$ sec  &$3.6$ sec&--&$2.1$ sec\\
  $9.3$ nats &$11$ nats&$2.3$ nats&--&$4.6$ nats
  \\\bottomrule
\end{tabular}\caption{Symbolic regression. Top: \% held out tasks solved w/ 5 second timeout. Middle: average solve time. Bottom: average description length $-\log \probability[x|p]$. ``No NN'' solves almost as many tasks but
using worse programs w/ extraneous degrees of freedom, increasing  description length.}\label{regressionComparison}
\end{table}
...
Instead, we provide array/string indexing primitives (\texttt{slice}, \texttt{nth}),
basic arithmetic, and functions for splitting a string into a list (\texttt{split})
or turning a list into a string (\texttt{join}).

We automatically generated  600 string editing along with input/output examples (Figure~\ref{exampleTextProblem}), split into a 75/25 train/test split.
...
\begin{tabular}{lllll}
  ...
  $0.88\pm 0.04$ sec
  \\\bottomrule
\end{tabular}\caption{\% held out tasks solved and average solve time for string edit problems}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{c}
    ...
    \texttt{append}, \texttt{chr->str}, \texttt{trim}, \texttt{nth}\\
...
\\    Subset of learned DSL\\
    \begin{tabular}{l}
      \toprule
      $f_0($\texttt{c}$) = $\texttt{(+ (chr->str c))}\\
      $f_1($\texttt{i,c,s}$) = $      \texttt{(+ (nth i (split c s))}\\\hspace{2.55cm}\texttt{ (chr->str c))}\\
      $f_2($\texttt{s,c}$) = $\texttt{((}$f_0$\texttt{ c) (}$f_1$\texttt{ (1+ 0) c s))} \\
      \hspace{1cm}($f_2$: \emph{Extract substring delimited by character})
...
\begin{tabular}{| l | l | l |}
  ...
  has-head-in-tail & [4\, 5\, 7\, 4] & \texttt{true} \\
...
low-level primitives such as \texttt{mapi}, \texttt{reducei}, and
\texttt{if}, whereas the rich DSL includes some common structure that can be
built from these such as \texttt{all}, \texttt{filter}, and \texttt{slice}.
...
\pagebreak
 ...
 \section{Conclusion}
 We have contributed a model, \system, that learns to program by
 bootstrapping a domain-specific language for expressing the solutions
 to the tasks at hand.  Alongside this DSL, we train a bottom-up
