...
relies critically upon a hand-engineered \emph{Domain Specific Language} (DSL).
The DSL is an inventory of restricted programming primitives,
crafted by an engineer,
which imparts domain specific knowledge about the space of programs.
...
determine a distribution over programs
from which a latent program is sampled,
which finally determines the agent's observations.
...
in~\ref{graphicalModel}(a)?  An intuitive strategy is to exploit the
conditional independence of the programs conditioned on the DSL, and
so to alternate between estimating the DSL ($\mathcal{D},\theta$) and
estimating a posterior distribution over the programs by searching for
programs written using the current DSL.  In practice searching the
space of programs is difficult, so we use the recognition model to
learn how to efficiently search for programs in the current DSL.
...
This makes our work in instance of
 ...
 Alongside this generative model, our algorithm learns a bottom-up recognition model (Figure~\ref{graphicalModel}(b)).
 The recognition model is a neural network $q$ that regresses from programming tasks $x$
 to a distribution over programs.
