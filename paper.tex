...
\icmlauthor{Kevin Ellis}{equal,MIT}
\icmlauthor{Lucas Morales}{equal,MIT}
...
\begin{abstract}
  ...
  learned DSL.  We use our model to build circuits, solve
  symbolic regression problems, edit strings, and synthesize functions on lists, in each case
...
our goal is to jointly infer a single DSL (written $\mathcal{D}$)
and many programs (written $p$), one for each set of inputs/outputs (\emph{tasks}, written $x$).
...
tasks.  We take inspiration from the Wake/Sleep
algorithm~\cite{hinton1995wake}, as well as the
Exploration-Compression algorithm (EC) for bootstrap
learning~\cite{Dechter:2013:BLV:2540128.2540316}.  Wake/Sleep
alternates between training a generative model on samples from a
recognition model (our $q$), and training a recognition model on
samples from the generative model (our $\mathcal{D},\theta$).  EC
...
We combine these ideas into an inference strategy that iterates
through three steps: a \textbf{Wake} cycle uses the current DSL and
recognition model to search for programs that solve the tasks.  The
\textbf{Sleep-G} and \textbf{Sleep-R} cycles update the DSL and the
recognition model, respectively.  Crucially, these steps bootstrap 
off each other (Fig.~\ref{feeding}):

\\\noindent \textbf{Wake: Searching for programs.}  Our program search
is informed by both the DSL and the recognition model. When these
improve, we find more programs solving the tasks.
\\\noindent\textbf{Sleep-G: Improving the DSL.} We induce the DSL
from the programs found in the wake phase, aiming
...
more closely match the domain.\footnote{This is loosely biologically
    inspired by the formation of abstractions during sleep memory
    consolidation~\cite{DUDAI201520}}
\\\noindent \textbf{Sleep-R: Learning a neural recognition model.}  We
...
programs found by the search procedure during waking. As the DSL
improves and as search finds more programs, the recognition model gets
both more data to train on, and better data.\footnote{These two sources are also
...
generative model diagrammed by Fig.~\ref{graphicalModel}(a). Writing $J$ for the joint probability of $(\mathcal{D},\theta)$ and $X$, we want the $\mathcal{D}^*$ and $\theta^*$ solving:
...
for which we have effective approaches like the Inside/Outside algorithm~(Lafferty,~2000), %\cite{international2000derivation},
...
We task our model
with solving 200 symbolic regression problems, each a polynomial of
