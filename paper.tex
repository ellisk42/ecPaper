...
\newcommand{\endingSystem}{\textsc{DreamCoder}}
...
%  \node[latent] at (3.5,1.5) (tx){$\theta^{(x)}$};
  ...
  \end{tikzpicture}
  \caption{(a) DSL $\mathcal{D}$ generates programs $p$ by sampling DSL primitives with probabilities $\theta$ (Alg.~\ref{programGenerativeModel}). Observe tasks $x$ (program inputs/outputs). (b)  Neural network $q(\cdot )$, the \emph{recognition model}, regresses from $x$ to the parameters of a distribution over programs,
...
This wake/sleep style of inference is the reason why we call our approach \endingSystem.
...
Tbl.~\ref{textPrimitives} (top) lists the primitive functions provided to \endingSystem.
...
We automatically generated  600 string editing tasks along with input/output examples (Fig.~\ref{exampleTextProblem}). %, split  75/25 train/test.
...
easy to hard. Without this curriculum, we cannot get off the ground.
