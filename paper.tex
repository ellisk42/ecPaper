...
\begin{figure}
  ...
  \edge {d,t}{z};
  \plate {}{(tx)(z)(x)}{$x\in X$};
  \draw [->,dashed] (x.east) to[out = 0,in = -90] node(nn){} (tx.south);
  ...
  \end{tikzpicture}
  \caption{DSL $\mathcal{D}$ generates programs $p$ by sampling DSL primitives with probabilities $\theta$ (Algorithm~\ref{programGenerativeModel}). We observe tasks $x$, which are program inputs/outputs. A neural network $q(\cdot )$ called the \emph{recognition model} regresses from $x$ to a distribution over programs ($\theta^{(x)} = q(x)$). Solid arrows correspond to the top-down generative model. Dashed arrows correspond to the bottom-up recognition model.}\label{graphicalModel}
\end{figure}


Our algorithm is best understood as inference in a hierarchical Bayesian model (Figure~\ref{graphicalModel}: solid lines):
A DSL $\mathcal{D}$
determines a distribution with production probabilities $\theta$,
from which a latent program $p$ is sampled,
which finally determines the agent's observations, $x$.
This makes our work in instance of
...
In tandem we learn a neural network $q$ that regresses from observations to
programs by proposing a distribution over programs $\theta^{(x)} = q(x)$.
We will show that the interaction of the generative and recognition models
is important for solving these program induction problems.
...
\begin{itemize}
  \item Maximum-likelihood estimation of a PCFG \citep{DBLP:conf/icml/LiangJK10} with
    ...
    $q$;
    % although some of the Liang paper actually learns subexpressions and therefore DSL
...
\begin{figure}\centering
  \begin{tikzpicture}
    ...
    \node[align = center](DSL) at (6,4) {DSL: $\mathcal{D}$};
    \node[align = center](synthesis) at (4,2) {Search for \\programs: $p$};
    \node[ align = center](recognitionModel) at (6,0) {Recognition \\model: $q$};

    \draw [->,thick] (DSL.west) to[out = 180,in = 90] node[above,rotate = 45]{{\footnotesize Makes tractable}} (synthesis.north);
    \draw [->,thick] (synthesis.east) to[out = 0,in = -90] node[above,rotate = 70]{{\footnotesize Trains}} (DSL.south);
    \draw [->,thick] (synthesis.east) to[out = 0,in = 90] node[below,rotate = -60]{{\footnotesize Trains}} (recognitionModel.north);
    \draw [->,thick] (recognitionModel.west) to[out = 180,in = -90] node[below,rotate = -70]{{\footnotesize Makes tractable}} (synthesis.south);
    \draw [->,thick] (DSL.east) to[out = 0,in = 0] node[above,rotate = -90]{{\footnotesize Trains}} (recognitionModel.east);
  \end{scope}
    \end{tikzpicture}
