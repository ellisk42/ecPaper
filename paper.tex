...
logic gates and multiplexers (Figure~\ref{circuitTables}, top).
We compare  with four baselines on 250 held-out circuits:
...
\begin{tabular}{lllll}
  ...
  84\% solved&76\%& 63\%&62\%&62\%\\
  $0.9$ sec&  1.1&1.2&1.3&1.3
  \\\bottomrule
\end{tabular}
\caption{Top: \% held-out Boolean circuits solved by \system and baselines w/ 5sec timeout. Bottom: average search time.}
\end{figure}
...
\begin{figure}\centering
  ...
  \caption{Learning curves for symbolic regression. Red: \system. Green: no neural network. Notice how the model without the neural network eventually solves almost all the tasks, but does so using programs with lower likelihood (see Table~\ref{regressionComparison}).}\label{regressionLearningCurve}
\end{figure}
...
We automatically generated  600 string editing along with input/output examples (Figure~\ref{exampleTextProblem}), split  75/25 train/test.
...
\\    Subset of learned DSL\\
    \begin{tabular}{l}
      ...
      $f_3($\texttt{c,s}$) = $\texttt{(nth (1+ 0) (split c (nth 0 s)))}\\
...
KfvTh$>$Dbl$<$OIJso 	&DBL
 ...
 $f($\texttt{s}$) = $   \texttt{(}$f_3$\texttt{ (split '>' (upper s)) '<')}
    \end{minipage}
