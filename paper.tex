...
programs written using $\mathcal{D}$.  In practice searching the
space of programs is difficult, so we use the recognition model to
learn how to efficiently search for programs.
...
We apply \system to four domains:
 ...
 \begin{table}
    \begin{tabular}{ll}
   \toprule
   Domain&Subset of induced DSL\\\midrule
   \\\bottomrule
 \end{tabular}
   \end{table}
...
We frame this problem as maximum a posteriori (MAP) inference in the generative model diagrammed in Figure~\ref{graphicalModel}. We wish to maximize the posterior probability of $\mathcal{D}$:
\begin{equation*}
