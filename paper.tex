...
Here, we show how to use \system to infer programs with continuous, real-valued parameters.

Demonstrates how to integrate continuous parameters into a program using our framework.
$p:\mathbb{R}\to\mathbb{R}$
\begin{align*}
&\log   \probability\left[\{(x_i,y_i)\}_{i\leq N}|p\right]\stackrel{+}{ = }\log \int \mathrm{d}C\; p_C(C)\prod_{i\leq N}\text{Normal}(p(x_i)|y_i)\\
\text{\emph{BIC:}} &\approx \sum_{i\leq N}\log \text{Normal}(p(x_i)|y_i) - \frac{|C|\log N}{2}
  \end{align}
\begin{figure}
...
\begin{algorithm}[tb]
  ...
  \STATE body $\sim$ sample$(\mathcal{D},\theta,\{\text{var}:\alpha\}\cup\mathcal{E},\beta)$
   ...
   \STATE Sample $e\sim \text{primitives}$, w.p. $\propto\theta_e$ if $e\in \mathcal{D}$
   \STATE \hspace{3.35cm}w.p. $\propto\frac{\theta_{var}}{|\text{variables}|}$ if $e\in \mathcal{E}$
