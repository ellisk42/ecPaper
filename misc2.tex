...
\author{Kevin Ellis, Mathias Sabl\'e Meyer, Lucas Morales, Maxwell Nye, Luke Hewitt,\\ Armando Solar-Lezama, Joshua B. Tenenbaum} % Author(s)
...
\begin{block}{Wake: Problem solving}
  \begin{tikzpicture}[scale=2.5,line width=1.3mm]
    \node at (0,0) (d){DSL};
...
\begin{column}{\twocolwid} % The first column within column 2 (column 2.1)
  ...
  12-minutes-timeout run.

\end{column}
\begin{column}{\onecolwid} % The first column within column 2 (column 2.1)
  The agent controls a `pen' on a 2D plant. Primitives are those of the LOGO
  language, modified to have a functional programing structure.
\end{column}
\end{columns}
