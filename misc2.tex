...
\title{\huge \systemEnding: Growing Libraries of Concepts with Wake-Sleep Program Induction} % Poster title

\author{Kevin Ellis, Mathias Sabl\'e Meyer, Lucas Morales, %% Maxwell Nye, Luke Hewitt,
  \\ Armando Solar-Lezama, Joshua B. Tenenbaum} % Author(s)
...
\begin{tikzpicture}[scale=4,line width=1.5mm]
  ...
  \node[latent,scale=3] at (3.5,3) (dx){DSL};
  \node[latent,scale=3] at ([yshift=-1.5cm,xshift=2cm]dx) (zp){prog};
  \node[obs,scale=3] at ([yshift=-1.45cm]zp) (xp) {data};
  \node[latent,scale=3] at ([xshift=2cm]zp) (zp1){prog};
  \node[obs,scale=3] at ([xshift=2cm]xp) (xp1) {data};
  ...
  \node at ([xshift=0.5cm]uses.east) (nnn){\NeuralNetwork{0.5}};
  \draw[thin] ([xshift=1.9cm,yshift=0.4cm]dx.east) rectangle
  ([xshift=0.5cm,yshift=-0.3cm]nnn.south);

  \end{tikzpicture}
  
  Observe program induction problems, 
  Solve  w/ latent programs.
  %% \textbf{Likelihood model} $\probability[x_n|p_n]$
  %% scores program $p_n$ on task $x_n$
  Latent  \textbf{DSL} acts as generative model over programs: `library of concepts'.
{\color{red}  \textbf{Recognition model}}: a neural net that predicts distribution over programs conditioned on task: `procedural knowledge'

\end{block}

\begin{block}{Wake: Problem solving}
  \begin{tikzpicture}[scale=2.5,line width=1.3mm,arrowhead=0.1mm]
    \node at (0,0) (d){DSL};
    \node at ([yshift = -2cm]d) (t){$\text{Data}$};

    \node at ([xshift = 2cm]t) (nn){
      \begin{tikzpicture}[x=2.5cm,y=1.25cm,transform canvas={scale=0.4,shift={+(-1,2.5)}}]
        \tikzstyle{neuron}=[circle,fill=blue!50,minimum size=20pt]
        \fill[fill=white] (-0.25,-0.5) rectangle (2.25,-4.5);
        \node[rectangle] at (1,1) {};
        \foreach \name / \y in {1,...,4}
    ...
    \node[align = center, text width = 1cm] at ([yshift = 0.6cm,xshift=-0.5cm]nn.north) {\baselineskip=0pt \small Recog. model\par};
    \draw [double,-{>[scale=0.2]}] (t.east) -- ([xshift = -0.5cm]nn.west);
...
%    \node at ([xshift=-7pt,yshift=5pt]s.north west) {$\mathcal{D}$};
    ...
    \draw [double,->] ([xshift = 0.5cm]nn.east) -- ([yshift = -0.25cm]s.west);
    \draw [->,rounded corners,double,] (d.east) -- ([yshift = 2cm]nn.center) -- ([yshift = 0.25cm]s.west);

    \node[align=left] at (7,-1) (f) {Programs};
    \draw [double,->  ] (s.east) -- (f.west);

    \draw [->  ,rounded corners,double,] (t.south) -- ([yshift = -0.5cm]t.south) -- ([yshift = -0.5cm] s.south |- t.south) -- (s.south);
  \end{tikzpicture}
...
\begin{block}{Sleep-R: Procedural knowledge}
  ...
  %% Simple model: just predicts probabilities of DSL productions.  
...
\begin{column}{\threecolwid} % The first column within column 2 (column 2.1)
...
\begin{block}{Sleep-G: Growing a DSL}
...
\begin{column}{\twocolwid} % The first column within column 2 (column 2.1)
  ...
  %% fashion
 (training set). % for the \system algorithm.
  ...
  %% aspect of the drawing part, or the pen,
  from blue (origin) to purple (end).

\end{column}
\begin{column}{\onecolwid} % The first column within column 2 (column 2.1)
  Agent controls a `pen' on a 2 dimentional plane. LOGO/Turtle primitives.%% Primitives are those of
...
%%   likelyhood function is all-or-none.
  ...
  Deep embedding of invented abstractions: up to $7$, %observed level of embedding of invented primitives on the 2 hours
  %% run was $7$
