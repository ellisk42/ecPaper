...
\begin{alertblock}{Wake/Sleep DSL Induction}
  \flushleft
  \textbf{DSL:}
  Domain Specific Language, a finely-tuned program representation, specialized to a domain of programming tasks.
...
\begin{block}{Bayesian framing}
  ...
  \includegraphics{figures/animation/DC.eps}
...
Train on two sources of data:
  ...
  \begin{itemize}
  \item \textbf{``Dreams'' from DSL}: Can sample unlimited data, quality
    positively correlates with fitness of generative model $\mathcal{D}$. Like Helmholtz Machine's recognition model training. Loss:
    \begin{equation*}
