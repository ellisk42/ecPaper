...
The program synthesis and AI communities have built many successful program synthesis algorithms,
...
list processing;
text editing (in the style of FlashFill~\cite{gulwani2011automating});
and symbolic regression.
For each of these we initially provide a generic set of programming primitives.
Our algorithm then discovers its own DSL for expressing solutions in the
domain (Tbl.~\ref{initialExampleDSL}).
%% \begin{figure}
%%   \begin{tabular}{lcl}
%%     \toprule
...
%%     \\Regression&\includegraphics[width = 1cm]{figures/functions/476.png}&\begin{tabular}{l}
%%       \code{(+ (* real x) real)} \\
...
$f(\text{\code{s}}) = $\code{(}$f_2$\code{ s)}\\
  ...
  \begin{tabular}{l}
    % $f_0($\code{r}$,\ell) \,=\, $\code{(fold $\ell$ r ($\lambda$ (x a)}\\
    % \phantom{$f_0($\code{r}$,\ell) \,=\, $\code{(fold}}\code{(cons (index (length a) $\ell$) a)))}\\
    ...
    $f_1(\ell,$\code{k}$) \,=\, $\code{($f_0$ (cons k empty) $\ell$)}\\
    \vspace{2ex}
      \hspace{0.5cm}($f_1$: \emph{Append }\code{k}\emph{ to $\ell$})\\
    $f_2(\ell,$\code{k}$) \,=\, $\code{(foldr $\ell$ (empty$?$ $\ell$)}\\
  ...
  \begin{tabular}{l}
      ...
      $f_2($\code{s}$) \,=\, $\code{(unfold s empty$?$ car }\\\hspace{1cm}\code{($\lambda$ (z) (}$f_1$\code{ z \codechar{ })))}\\
...
\end{tabular}}
\caption{Top: Example tasks from each domain, each followed by the programs
  \system discovers for them. Bottom: Subset of learned DSL. Notice that
  learned DSL primitives can call each other
  ($f_0$/$f_1$ in Lists, $f_1$/$f_2$ in Text \& Regression)}
\end{figure}
...
then
an effective algorithm for learning DSLs which jointly trains a neural net to
...
subexpressions of programs to $\mathcal{D}$.  Instead, we are
...
After three iterations, it assembles a DSL (Tbl.~\ref{textPrimitives}) that lets it rapidly explore the space of programs and find solutions to
all of the tasks.
...
\begin{tabular}{llllll}
  ...
  \\\midrule\multicolumn{6}{c}{\emph{List Processing}}\\\midrule
  \% solved&\textbf{86\%} &84\% &60\%&74\%&70\%\\
  Solve time&  0.8s&0.7s&1.0s&1.0s&1.1s

  \\\midrule\multicolumn{6}{c}{\emph{Text Editing}}\\\midrule
