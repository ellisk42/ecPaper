...
\begin{minipage}{0.6\textwidth}
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \captionof{table}{Sample tasks from our list function domain}
  \label{listexamples}
\end{minipage}
\hfill
\begin{minipage}{0.6\textwidth}
  ...
  \begin{tabular}{l}
    \toprule
    $f_0($\code{i$,\ell$}$) \,=\, $\code{(TODO)}\\
    \hspace{0.5cm}($f_0$: \emph{TODO})\\
  \bottomrule
  \end{tabular}
  \captionof{table}{Some learned DSL primitives for list functions}\label{listinduced}
\end{minipage}
  }
\end{figure*}
...
\system learns a DSL containing templates for polynomials of different orders, as well as ratios of polynomials (Tbl.~\ref{regressionDSL}).
...
\begin{minipage}{0.5\textwidth}
  ...
  \\\bottomrule
\end{tabular}
  \captionof{table}{Some learned subroutines for symbolic regression. System starts with addition, multiplication, division, and real numbers, and learns to build rational functions and polynomials up to 4th order.}\label{regressionDSL}
\end{minipage}
}
\end{figure*}
