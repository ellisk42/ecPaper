...
because it iterates between three different phases: an
\textbf{Explore} phase uses the DSL to explore the space of programs,
searching for ones that solve the tasks; a \textbf{Compress} phase
modifies the DSL by discovering regularities in the programs found by
the previous Explore phase; and a \textbf{Compile} phase, which
...
We apply \system to three domains:
 ...
 For each of these we provide a generic
...
through three steps: an \textbf{Explore} cycle uses the current DSL and
recognition model to search for programs that solve the tasks.  The
\textbf{Compress} and \textbf{Compile} cycles update the DSL and the
...
programs.  As we solve more tasks, we hone in on richer DSLs that
...
\begin{wrapfigure}{r}{0.6\textwidth}
  ...
  \end{scope}
    \end{tikzpicture}
  \caption{\system solves for programs, the DSL, and a neural network called the \emph{recognition model}. Each of these steps bootstrap off of the others in an iterative inference algorithm.}  \label{feeding}
\end{wrapfigure}
...
\begin{align}\label{intractableObjectives}
  J(\mathcal{D},\theta)\triangleq \probability[\mathcal{D},\theta]\prod_{x\in X} \sum_p \probability[x|p]\probability[p|\mathcal{D},\theta],\quad
  \mathcal{D}^* = \argmax_{\mathcal{D}}\int J(\mathcal{D},\theta)\;\mathrm{d}\theta, \quad
...
posterior over programs, $q(p|x)\propto\probability[x,p|\mathcal{D},\theta}]$,
...
\begin{align}\nonumber
\mathcal{L}_{\text{HM}} = \expect_{p\sim(\mathcal{D},\theta) }\left[\log q(p|x)\right]\text{ s.t. $x = \sem{p}$}\quad
...
\centering  \begin{tabular}{ll}
    ...
    \begin{tabular}{l}
      \code{(lambda (a b) (fold b (cons "," a) (lambda (x z) (cons x z))))}\\
      \code{(lambda (a b) (fold a b (lambda (x z) (cons x z))))}
    \end{tabular}&
    \code{(fold a b (lambda (x z) (cons x z)))}\\\bottomrule
  \end{tabular}
...
\begin{algorithm}[tb]
   ...
   \begin{algorithmic}
     ...
     \STATE $q\gets \text{train recognition model, maximizing }\mathcal{L}_{\text{RM}}$ (\footnotesize{\textbf{Compile}})
...
\begin{wrapfigure}{r}{0.3\textwidth} \newcommand{\functionSize}{0.75cm}
  ...
  \includegraphics[width = \functionSize]{figures/functions/303.png}\\  
  \includegraphics[width = \functionSize]{figures/functions/402.png}
  ...
  \end{wrapfigure}We show how to use \system to infer programs containing both discrete
...
We task our model
with solving 200 symbolic regression problems, each either a polynomial of
degree 1 to 4 or a rational function.
The recognition model is a convolutional network that observes an image
of the target function's graph (Fig.~\ref{functions}) --
visually, different kinds of polynomials and rational functions
produce different kinds of graphs,
and so the recognition model can learn to
look at a graph and predict what kind of function best explains it.


We initially give our system addition, multiplication, and division,
...
\system learns a DSL containing templates for polynomials of different orders,
which lets the algorithm quickly find the functions that are most appropriate to this domain (Tbl.~\ref{regressionDSL}).
We also found that the algorithm discovers programs that minimize the number of continuous degrees of freedom.
For example, it represents the linear function $-x+2$ with the program
\code{(* (+ x }$\mathcal{R}$\code{) }$\mathcal{R}$\code{)} which has two continuous degrees of freedom, and represents quartic functions using the invented DSL primitive $f_6$ in Tbl.~\ref{regressionDSL}
...
We apply \system to text editing (Section~\ref{textSection}) and list processing (Section~\ref{listSection})  problems.
...
The initial set of primitives includes \code{fold}, \code{unfold}, \code{if}, \code{map}, \code{length}, \code{=}, \code{+}, \code{-}, \code{0}, \code{1}, \code{cons}, \code{car}, \code{cdr}, \code{nil}, and \code{is-nil}.


\subsubsection{String Editing}\label{textSection}
...
We show \system can instead start out with generic primitives
...
\begin{table}[t]
  ...
  \\\bottomrule
    \end{tabular}
  \caption{Some learned DSL primitives for string editing}\label{textPrimitives}
  \end{table}
...
\multicolumn{2}{l}{$f(\text{\code{a}},\text{\code{b}}) = $\code{(}$f_0$\code{ a (cons " " b))}}
\\
\bottomrule
  \end{tabular}
...
\subsubsection{List Functions}\label{listSection}
...
We contribute a model, \systemEnding, that learns to program by bootstrapping a DSL with new
