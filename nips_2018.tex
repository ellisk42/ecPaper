...
%% where  is large.
\\\noindent \textbf{Compress: Maxing $\int \lowerBound\;\mathrm{d}\theta$ w.r.t.\ the DSL.} Here $\left\{\mathcal{F}_x \right\}_{x\in X}$ is held fixed, and so we can evaluate $\lowerBound$. Now the problem is that of searching the discrete space of DSLs and finding one maximizing $\int \lowerBound\;\mathrm{d}\theta$.
...
during the Compile step: $q$ is trained to approximate the
...
\textbf{Compile: learning to tractably maxing $\lowerBound$ w.r.t. the
...
We train $q$ both on programs found during the Explore step and
on samples from the current DSL.

\begin{wrapfigure}{r}{0.6\textwidth}\vspace{-0.55cm}\centering
  \begin{tikzpicture}[scale=0.9]
    ...
    \draw [->,thick] (synthesis.-120) to[out = -150,in = 60] node[below,rotate = 45,align = center]{{\footnotesize Trains}\\{\footnotesize (Compress)}} (DSL.30);
    \draw [->,thick] (synthesis.-60) to[out = -30,in = 120] node[below,rotate=-45,align = center]{{\footnotesize Trains}\\{\footnotesize (Compile)}} (recognitionModel.150);
    \draw [->,thick] (DSL.east) to[out = -30,in = 210] node[above, align = center]{{\footnotesize Trains}\\{\footnotesize (Compile)}} (recognitionModel.west);

    \draw [->,thick,dashed] (DSL.north) to[out = 90,in = 180] node[fill=white,align = center]{  \footnotesize{Inductive bias}\\\footnotesize{(Explore)}} (synthesis.west);
    \draw [->,thick,dashed] (recognitionModel.north) to[out = 90,in = 0] node[fill=white,align = center]{{\footnotesize Makes tractable}\\{\footnotesize (Explore)}} (synthesis.east);
  \end{scope}}
    \end{tikzpicture}
  \caption{How these steps bootstrap each other.}  \label{feeding}\vspace{-1cm}
\end{wrapfigure}%
Crucially, each of these three steps bootstraps the others (Fig.~\ref{feeding}): improving either the DSL or the recognition model
makes search easier, so we find more programs solving tasks;
both improving the DSL and solving more tasks expands the training data for the recognition model;
and finding more programs that solve tasks
gives more data from which to learn a DSL.


%% With $q$ in hand we can find programs for
...
\vspace{0.5cm}Putting all these ingredients together, Alg.~\ref{mainAlgorithm} describes how we combine program search,
recognition model training, and DSL induction.
For added robustness,
we interleave an extra program search step (Explore) before training the recognition model,
and just enumerate from the prior $(\mathcal{D},\theta)$ during this extra Explore step.
