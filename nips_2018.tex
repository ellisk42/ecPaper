...
like these, the program synthesis and AI communities have had many successes,
...
\begin{tabular}{>{\hspace{-1em}}c<{\hspace{-1em}}>{\hspace{-1em}}c<{\hspace{-1em}}>{\hspace{-1em}}c<{\hspace{-1em}}}\toprule
  Lists&Strings&Symbolic Regression\\\midrule
  \begin{tabular}{c}
  ...
  \begin{tabular}{c}
    ...
    $f(\text{\code{s}}) = $\code{(}$f_0$\code{  \codechar{.} \codechar{,} ($f_0$ \codechar{.} \codechar{ } (cdr s)))}\\\\
Temple Anna H $\to $TAH\\
Lara Gregori$\to $LG\\
$f(\text{\code{s}}) = $\code{(}$f_2$\code{ s)}\\
  ...
  \begin{tabular}{l}
    ...
    $f_0(\ell) \,=\, $\code{(foldr $\ell$ 0 ($\lambda$ (x a) (if (> a x) a x)))}\\
      \hspace{0.5cm}($f_1$: \emph{Get the largest number in $\ell$})\\
    $f_1(\ell,$\code{k}$) \,=\, $\code{(foldr $\ell$ (empty$?$ $\ell$)}\\
    \phantom{$f_1(\ell,$\code{k}$) \,=\, $\code{(foldr}}
    \code{($\lambda$ (x a) (if a a (= k x))))}\\
      \hspace{0.5cm}($f_1$: \emph{Whether $\ell$ contains $k$})\\
  \end{tabular}&
...
the best-performing algorithm solves 79.6\% problems.
