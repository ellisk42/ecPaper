...
\title{Library Learning in Neurally-Guided Bayesian Program Learning}
...
\author{
  Kevin Ellis\\MIT\\\texttt{ellisk@mit.edu}\And Lucas Morales\\MIT\\\texttt{lucas@lucasem.com}\And Mathias Sabl\'e-Meyer\\ENS Paris-Saclay\\\texttt{mathsm@mit.edu}
  \AND
  ...
  \begin{minipage}{0.6\textwidth}    
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}
        ...
        \footnotesize{\hspace{0.0cm}(\textbf{Explore})}
...
causing it to hit a `plateau' after which it stops solving new tasks after an initial spurt of learning (Fig.~\ref{learningCurves}) -- without the neural network, search becomes intractable.
This `plateau' supports a view of the recognition model as a way of amortizing the cost of search.
...
\subsubsection*{Acknowledgments} We are grateful for collaborations with Eyal Dechter, whose EC algorithm directly inspired this work. Funding from NSF GRFP and the MUSE program (Darpa grant FA8750-14-2-0242).
