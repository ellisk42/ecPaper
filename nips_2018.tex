...
\begin{tabular}{lcccccc}
  ...
  \midrule
  \% solved&\textbf{79\%}\footnote{A last-minute bug caused this  to run without ``Helmholtz machine'' style training ($\mathcal{L}_{\text{RM}} = \mathcal{L}_{\text{SS}}$). We are rerunning this experiment and expect this number to be higher.} &
  76\% &71\%&35\%&62\%&37\%\\
...
only add \textbf{s}ub\textbf{e}xpressions of programs in the frontiers to the DSL. This is how prior approaches have learned libraries of functions~\cite{Dechter:2013:BLV:2540128.2540316,DBLP:conf/icml/LiangJK10,DBLP:conf/ecai/LinDETM14}.
...
\begin{figure}\centering
  ...
  \caption{Learning curves for \system both with (\popp{orange}) and without
  (\pop{teal}) the recognition model. Solid lines: \% holdout testing tasks solved. Dashed lines: Average solve time.}\label{learningCurves} 
\end{figure}
...
preliminary results for turtle graphics  --- see supplementary material for more
...
\begin{minipage}{0.4\textwidth}
 ...
 Bottom: learning curve  (middle), and on both sides the average sample from the DSL
  before any training (left) and after training (right). See Sec.~1 of supplement.}
\label{geomCompiled}
\end{minipage}
