...
We `batch' the tasks by randomly shuffling the training set
and playing small minibatches
of tasks to the agent during each wake cycle.
...
inspiration from LOGO Turtle graphics~\cite{turtle}, tasking it with
drawing a modest corpus of images while equipping it with control over
a `pen', along with arithmetic operations on angles and distances.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{Output:} Infinite stream of DSLs, recognition models, and frontiers
    \State \textbf{Hyperparameters:} Batch size $B$, enumeration timeout $T$, maximum frontier size $F$
    \State $\theta\gets $uniform distribution
    \State $\mathcal{F}_x\gets\varnothing $, $\forall x\in X$\Comment{Initialize frontiers to be empty}
...
\begin{center}
  \begin{tabular}{ccccccc}
    \toprule
    Domain&Timeout&CPUs&Batch size&$\lambda$ (Sec.~\ref{consolidationSection})&$\alpha$ (\ref{mapAppendix})&Max frontier (\ref{systemPseudocode})\\\midrule
