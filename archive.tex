...
\begin{figure}\centering
  \begin{tikzpicture}
    ...
    \node[align=center,rotate=90] at ([xshift=1cm]snow.east) {$\underbrace{\hspace{6cm}}_{\text{\normalsize \textbf{\textsc{Dreams}: Samples from DSL}}}$};
    
    \node at ([xshift=3cm]semi.east) {\includegraphics[width = 1.5cm]{figures/finalDreams/63smooth_pretty}};
    \node at ([xshift=3cm]snow.east) {\includegraphics[width = 1.5cm]{figures/finalDreams/400smooth_pretty}};
    \node at ([xshift=3cm]line.east) {\includegraphics[width = 1.5cm]{figures/finalDreams/399smooth_pretty}};
  \end{tikzpicture}
...
and acts as a lower bound on the joint:
\begin{align}
...
%% which we can accomplish by adding new programs to the beams means searching for new programs $p$ for task $x$
%% where  is large.
\\\noindent \textbf{Sleep (Consolidation): Maxing $ \lowerBound$ w.r.t.\ the DSL.} Here $\left\{\mathcal{B}_x \right\}_{x\in X}$ is held fixed, and so we can evaluate $\lowerBound$. Now the problem is that of searching the discrete space of DSLs and finding one maximizing $\int \lowerBound \;\mathrm{d}\theta$,
and then updating $\theta$ to $\argmax_\theta \lowerBound(\mathcal{D},\theta,\left\{\mathcal{B}_x \right\})$.
...
\probability[\theta|\mathcal{D}] + \|\theta\|_0}_{\text{Description length of }(\mathcal{D},\theta)} +\sum_{x\in
  ...
  \mathcal{B}_x}\probability[x|p]\probability[p|\mathcal{D},\theta]}_{\text{Description length of programs for task }x}\Bigg)
\label{AIC}
  \end{equation}
...
the programs found during waking ($\left\{\mathcal{B}_x \right\}$)
...
\begin{figure}
  ...
  \end{tabular}
  \caption{Test set performance across wake/sleep iterations. An Epoch is a full pass over the set of training tasks. Orange: \systemEnding. Teal: No recognition model. Purple: No DSL learning. Solid lines: \% tasks solved within 10 minute timeout. Dashed lines: average search time. Shaded region is $\pm 1$ stddev search time.}\label{learningCurves}
\end{figure}
...
more broadly useful for AI.
Many challenges remain.

How could a program induction approach to
...
This is similar in spirit to iterative deepening depth first search~\cite{Russell:2003:AIM:773294}.
 ...
 the machinery of version spaces (Appendix~\ref{appendixVersion})
 and a probabilistic objective favoring compressive DSLs.
...
\begin{algorithm}%[H]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\forall x\;:\;\mathcal{B}_x\gets\left\{\text{\textsc{refactor}}(p|\mathcal{D})\;:\; p\in \mathcal{B}_x\right\}$\Comment{Refactor beams in terms of new DSL}
    \EndWhile
  \end{algorithmic}
\end{algorithm}
...
%%       \end{tabular}
    ...
    \\\bottomrule 
    \end{tabular}
  \caption{Bootstrapping a standard library of functional programming routines, starting from recursion along with primitive operations found in 1959 Lisp. Complete set of tasks and learned DSL shown. Learned DSL components are numbered in the order that they are learned.}\label{learningFromScratch}
  \end{figure}
...
\begin{center}
  \begin{tabular}{ccccccc}
    ...
    Graphics&1h&128&40&1&30&5\\
    Symbolic regression& & 40&10 & 1&30&5\\
    Towers& & 64&10 & 1&30&5\\
    Regexes& & 64&10 & 1&30&5\\
    \bottomrule     
  \end{tabular}
\end{center}
