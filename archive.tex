...
\begin{figure}
  \begin{tikzpicture}[line width=0.4mm]
    ...
    \begin{scope}[shift={(0.5,0.5)}]
      \node[align=right] at (0,0) (d){\textbf{DSL}\\
        $f_1$, $f_2$, ...};
    ...
    \node at (0.5,-4.75) {\textbf{\textsc{Sleep: Consolidation}}};
    \draw[fill=teal!5!white] (-2,-5) -- (3,-5) -- (3,-10) -- (5.5,-10) -- (5.5,-13) -- (-2,-13) -- (-2,-5);
    \node at (11.5,-4.75) {\textbf{\textsc{Sleep: Dreaming}}};
    \draw[fill=teal!5!white] (14,-5) -- (9,-5) -- (9,-10) -- (6.5,-10) -- (6.5,-13) -- (14,-13) -- (14,-5);
    ...
    \begin{scope}[shift={(9,-4.5)}]
      ...
      \begin{scope}[shift={(-3.3,-8)},scale=0.6,line width=0.05mm]
        \node[obs,scale=0.7] at (3.5,3) (dx){DSL};
    ...
    \begin{scope}[shift={(-2,-4.5)}]
      \node[align=center] at (0.8,-1.2) (f1){progs. for\\task$_1$:\\\mcode{(+ (car z) 1)}};
      \node[align=center] at ([xshift = 1cm]f1.east) (f2){progs. for\\task$_2$:\\\mcode{(cons (+ 1 1))}};
      \node[align=center] at ([yshift = 0cm,xshift=0.4cm]f2.east) (f3){\textbf{DSL}\\\mcode{cons}, \mcode{+}, \\...};
      \node(c)[rectangle, rounded corners, dashed, draw, minimum width = 3.75cm, minimum height = 3cm, anchor = north] at ([yshift=-0.8cm,xshift=-0.3cm]f2.south) {};
      \draw [-> ] (f1.south) -- (f1.south|-c.north);
      \draw [-> ] (f2.south) -- (f2.south|-c.north);
      \draw [-> ] (f3.south) -- (f3.south|-c.north);

      
      \node(d) at ([yshift = -1.8cm]c.south) {
        \begin{tikzpicture}[scale=0.9,line width=0.5mm]
          \node[align=center] at (0,0) {DSL w/ \code{(+ x 1)}:};
      ...
      \node at (c.center) {
        \begin{tikzpicture}[scale=0.9,line width=0.35mm]
...
%    \draw (0,0) rectangle (10,5);
    \node(t1)[align=left] at (1,-1) {
      ...
      \footnotesize $f(\text{\mcode{s}}) = $\mcode{(}\subnode{f01}{${\color{orange}f_0}$}\mcode{  \codechar{.} \codechar{-}}\\
      \footnotesize \hspace{1.25cm}\mcode{(${\color{orange}f_0}$ \codechar{.} \codechar{ }}\\
      \footnotesize \hspace{1.4cm}\mcode{(cdr s)))}
    };
    \node(t2)[align=left] at ([xshift=2cm]t1.east) {
      ...
      \footnotesize $f(\text{\mcode{s}}) = $\mcode{(${\color{orange}f_3}$ (${\color{orange}f_4}$ \codechar{ } s))}
    };
    \node(t3)[align=left] at ([xshift=2cm]t2.east) {
      ...
      \footnotesize $f(\text{\mcode{s}}) = $\mcode{(${\color{orange}f_6}$ \codechar{(} \codechar{)} s)}
    };

    \draw (-2,-2.5) -- (15,-2.5);
    \node[align=left,rotate=90] at (-1.5,-1.3) {Tasks and\\\\\texttt{Programs}};

    \node[align=left,rotate=90] at (-1.5,-6.3) {Domain Specific Language (DSL)};

    \renewcommand{\horizontalSpacing}{3.5cm}
    ...
    \invention{substitution}{1,-4}{
      ${\color{orange}f_0}($\mcode{s}$,$\mcode{a}$,$\mcode{b}$) \,=\, $\\
       \mcode{(map ($\lambda$ (x) (if (= x a)\\ b x)) s)}
    }{substitute chars}
    \vinvention{cat}{substitution}{
      ${\color{orange}f_1}($\mcode{a}$,$\mcode{b}$) \,=\, $\\
      \mcode{(fold ($\lambda$ (x y)\\ (cons x y))\\ a b))}
    }{concat strings}
    \vinvention{looping}{cat}{
       ${\color{orange}f_2}($\mcode{c}$,$\mcode{p}$,$\mcode{s}$) \,=\, $\\
      \mcode{(unfold l ($\lambda$ (x) (= c (car x))) p ($\lambda$ (z) (cdr z)))}
    }{loop until char}

    \hinvention{constant}{substitution}{
      ${\color{orange}f_3}($\mcode{s}$) \,=\, $\\
      \mcode{(${\color{orange}f_1}$ s string)}
    }{prepend constant}
    \vinvention{drop}{constant}{
      ${\color{orange}f_4}($\mcode{c}$) \,=\, $\\
      \mcode{(${\color{orange}f_2}$ c ($\lambda$ }\\\mcode{(z) (car z)))}
    }{prefix}
    \vinvention{suffix}{drop}{
      ${\color{orange}f_5}($\mcode{s}$,$\mcode{c}$) \,=\, $\\
      \mcode{(fold ($\lambda$ (x y) }\\\mcode{(cdr (if (= x c)}\\\mcode{ s y))) s s)}
    }{suffix}

    \hinvention{delimited}{constant}{
      \mcode{${\color{orange}f_6}($x$,$y$,$s$)\,=\,$}\\
      \mcode{(${\color{orange}f_4}$ x (${\color{orange}f_5}$ y c))}
    }{delimited substring}
    \draw[->] (delimited.west) -- (drop.east);
    \draw[->] (delimited.west) -- (suffix.east);

    \vinvention{appendSingle}{delimited}{
      \mcode{${\color{orange}f_7}($c$,$s$)\,=\,$}\\
      \mcode{(${\color{orange}f_1}$ (cons c nil) s)}}{append character}
    
\vinvention{surround}{appendSingle}{
      \mcode{${\color{orange}f_8}($s$)\,=\,$}\\
      \mcode{(cons \codechar{(} (${\color{orange}f_7}$ \codechar{)} s))}}{surround in parens}
    ...
    \begin{scope}[shift={(0,-15)}]
      \invention{filter}{0,0}{
        ${\color{orange}f_0} = $\mcode{($\lambda$ (p l)\\ (fold ($\lambda$ (x a)\\ (if (p x) (cons x a) a))\\ nil l))} 
        }{higher-order `filter'}

      \invention{three}{[yshift=-\verticalSpacing]filter.center}{\footnotesize ${\color{orange}f_1} = $\\\mcode{(+ 1 (+ 1 1))}}{The number `3'}
      \invention{prefix}{[yshift=-\verticalSpacing]three.center}{\footnotesize ${\color{orange}f_2} = $\mcode{($\lambda$ (n l) (map ($\lambda$ (i) (index i l)) (range (+ 1 n))))}}{First $n$ elements}
      \invention{single}{[xshift=\horizontalSpacing]filter.center}{\footnotesize ${\color{orange}f_3} = $\mcode{($\lambda$ (z l) (fold ($\lambda$ (x y) (cons x y)) z l))}}{Append element}
      \invention{for}{[yshift=-\verticalSpacing]single.center}{\footnotesize ${\color{orange}f_4} = $\mcode{(+ 1 ${\color{orange}f_1}$)}}{The number `4'}
      \invention{Max}{[yshift=-\verticalSpacing]for.center}{\footnotesize ${\color{orange}f_5} = $\mcode{($\lambda$ (l) %
          (car (${\color{orange}f_0}$ ($\lambda$ (x) (nil? (${\color{orange}f_0}$ l ($\lambda$ (z) (> z x))))) l)))}}{Max of list}
      \invention{largest}{[xshift=\horizontalSpacing]single.center}{\footnotesize ${\color{orange}f_6} = $\mcode{($\lambda$ (l n) %
          (${\color{orange}f_5}$ (${\color{orange}f_0}$ ($\lambda$ (x) (> n (length (${\color{orange}f_0}$ ($\lambda$ (y) (> x y)) l)))) l)))}}{$n^{\text{th}}$ largest element}
      \vinvention{count}{largest}{\footnotesize ${\color{orange}f_7} = $\mcode{($\lambda$ (x l) %
          (length (${\color{orange}f_0}$ ($\lambda$ (y) (= x y)) l)))}}{count}
      \vinvention{weird}{count}{\footnotesize ${\color{orange}f_8} = $\mcode{($\lambda$ (p l) %
          (map ($\lambda$ (x) (if (p x) (+ 1 x) 0))))}}
                 {Higher-order incrementer}

      \draw[->] (for.west) -- (three.east);
      \draw[->] ([yshift=-0.5cm]largest.west) -- (Max.east);
      \draw[->] ([yshift=-0.5cm]largest.west) to[out=240,in=340] ([yshift=-0.8cm]filter.east);
      \draw[->] (Max.west) -- (filter.east);
      \draw[->] ([yshift=0.1cm]count.west) to[out=90,in=300] ([yshift=-0.3cm]filter.east);

      \node(t1)[align=left] at (0,3) {
        \mcode{[1 4 5 8]$\to$[5 8]}\\
        \mcode{[9 8 3 8]$\to$[9 8 8]}\\
        \mcode{[3 4 5 6]$\to$[5 6]}\\
        \mcode{[6 5 4 3]$\to$[6 5]}\\
        \footnotesize $f(\text{\mcode{z}}) = $\mcode{(${\color{orange}f_0}$ ($\lambda$ (x)}\\\hspace{0.5cm}\mcode{ (> x ${\color{orange}f_4}$)) z)}
      };
      \node(t2)[align=left] at ([xshift=3.75cm]t1.center) {
        \mcode{[9 2 4]$\to$[4 2 9]}\\
        \mcode{[4 3]$\to$[3 4]}\\
        \mcode{[3 4 5 6]$\to$[6 5 4 3]}\\
        \mcode{[]$\to$[]}\\
        \footnotesize $f(\text{\mcode{z}}) = $\mcode{(fold ($\lambda$ (x a)}\\\hspace{0.25cm}\mcode{ (${\color{orange}f_3}$ x a)) nil z)}
      };
      \node(t3)[align=left] at ([xshift=3.75cm]t2.center) {
        \mcode{[9 2 3]$\to$[2 3 9]}\\
        \mcode{[2 1]$\to$[1 2]}\\
        \mcode{[8 7 9 2 5]$\to$[2 5 7 8 9]}\\
        \footnotesize $f(\text{\mcode{z}}) = $\mcode{(map ($\lambda$ (i)}\\\hspace{0.5cm}\mcode{ (${\color{orange}f_6}$ z i)) }\\\hspace{0.5cm}\mcode{(+ 1 (range (length z)))}
      };

      \draw[->] ([yshift=0.5cm,xshift=-0.2cm]t1.south) to[out=250] (filter.north);
      \draw[->] ([yshift=0.0cm,xshift=0.3cm]t1.south) to[out=350,in=100] (for.west);
      \draw[->] ([xshift=-0.7cm]t2.south) -- (single.north);
      \draw[->] ([xshift=-1.25cm,yshift=0.5cm]t3.south) to[out=240] (largest.north);
...
\begin{table*}%[t!]
  \makebox[\textwidth][c]{ \scriptsize \tabcolsep=4pt
    \renewcommand\code\texttt \renewcommand\codechar[1]{\texttt{"#1"}}
