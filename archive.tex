...
\begin{figure}
  \begin{tikzpicture}[line width=0.4mm]
    ...
    \draw[fill=white!5!white] (-2,-5) -- (3,-5) -- (3,-10) -- (5.5,-10) -- (5.5,-13) -- (-2,-13) -- (-2,-5);
    \node at (11.5,-4.75) {\textbf{\textsc{Sleep: Dreaming}}};
    \draw[fill=white!5!white] (14,-5) -- (9,-5) -- (9,-10) -- (6.5,-10) -- (6.5,-13) -- (14,-13) -- (14,-5);
    ...
    \begin{scope}[shift={(-1.9,-4.5)}]
      \node[align=center] at (0.8,-1.2) (f1){progs. for\\task$_1$:\\%
        \mcode{(+ (car z)}\vspace{-5pt}\\\mcode{1)  }};
      \node[align=center] at ([xshift = 0.7cm]f1.east) (f2){progs. for\\task$_2$:\\\mcode{(cons }\vspace{-5pt}\\\mcode{(+ 1 1))}};
      \node[align=center] at ([xshift=0.7cm]f2.east) (f3){\textbf{DSL}\\\mcode{cons}, \mcode{+}, \\...};
      \node(c)[rectangle, rounded corners, dashed, draw, minimum width = 3.75cm, minimum height = 3cm, anchor = north] at ([yshift=-0.8cm,xshift=-0.0cm]f2.south) {};
...
\begin{figure}
  \begin{tikzpicture}
    ...
    \renewcommand{\horizontalSpacing}{3.9cm}
    \renewcommand{\verticalSpacing}{2cm}
    
    \newcommand{\invention}[4]{
      \node(#1)[inner sep=1,anchor=south,text width=2cm,draw,rounded corners,align=center] at (#2) {
        \footnotesize #3
      };
    ...
    \node[anchor=center,align=center](t1) at (2,0) {
      \includegraphics[width = 2cm]{figures/logoTasks/logo144.png}\\
    \mcode{(for (i 4) (\use{3} i))}};
    \node[anchor=center,align=center](t2) at ([xshift=\horizontalSpacing]t1.center) {
      \includegraphics[width = 2cm]{figures/logoTasks/logo121.png}\\
    \mcode{(\use{7} 6 (\use{1} \use{0}))}};
    \node[anchor=center,align=center](t3) at ([xshift=\horizontalSpacing]t2.center) {
      \includegraphics[width = 2cm]{figures/logoTasks/logo133.png}\\
      \mcode{(\use{7} 7 (\use{8} \use{6}))}};
    \invention{circle}{2,-3}{
    ...
    \draw[->] (t1.south) -- (square.north);
    \draw[->] (t2.south) -- (circle.north);
    \draw[->] (t3.south) -- (semi.north);
    \draw[->] (t2.south) -- (snow.west);
    \draw[->] (t2.south) -- (pu.east);
    \draw[->] (t3.south) to[out=320,in=45] (line.east);
    \draw[->] (t3.south) to[out=200,in=120] (snow.west);

    \draw[->] (snow.west) -- (angle.east);
    \draw[->] (smooth.west) -- (repeated.east);
    \draw[->] (semi.west) -- (smooth.east);
    \draw[->] (square.west) -- (repeated.east);
  \end{tikzpicture}
