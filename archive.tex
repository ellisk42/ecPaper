...
\begin{figure}
  \includegraphics[width = 1.2\textwidth]{figures/DeepLogo_4_11}
...
basis, including routines like \code{map}, \code{fold}, \code{cons}, \code{car}, \code{cdr}, etc.
...
\begin{figure}
  \includegraphics[width = \textwidth]{figures/text_tsne.png}
  \caption{Left: TSNE embedding of model's `at-a-glance' (before searching for solution) organization of text tasks (neural net activations), after first wake/sleep cycle. Middle: at-a-glance organization after 20 wake/sleep cycles. Right: TSNE visualization of programs actually used to solve tasks; intuitively, how model organizes tasks after searching for a solution. More examples in Fig.~\ref{allTSNE}}\label{textExpertiseCluster}
\end{figure}
...
with drawing a corpus of 160 images while equipping it with control
...
function into its library (Figure~\ref{logoPrimitives}, right).
...
\begin{figure}
\centering  \begin{tabular}{cc}
    \toprule Example learned DSL components&Samples written using final library\\\midrule
    \begin{tabular}{rl}
  ...
  \caption{\textbf{Left}: Three (out of 19) learned DSL components for building towers out of Lego-style blocks. These components act like parametric options~\cite{stolle2002learning},
    giving higher-level building blocks that the agent can use to plan. \textbf{Right}: 16 random samples, or `dreams', built from learned library.}\label{tower}
\end{figure}
...
number of strings, where these strings are drawn from CSV columns crawled from the web~\cite{LUKE}. Across wake/sleep cycles, our agent learns to learn
...
We first consider learning a basic language for equations describing physical laws,
starting \emph{without} concepts like vectors or forces
and instead providing Lisp-style sequence manipulation primitives like
\code{map} and \code{fold}.
We collected 57 equations for physical laws from AP and MCAT physics `cheat sheets' --
forms like Coulomb's law and expressions for the center of mass of a set of objects --
along with
reference vector algebra identities from machine learning textbooks,
and tasked our agent with inferring programs calculating these input/output mappings.
...
and then uses this vocabulary to infer library routines for common
physical relationships like inverse square laws and integrating second derivatives over time.
...
\begin{figure}
  ...
  \begin{tabular}{lll}\toprule
    ...
    \begin{tabular}{l}
      ...
      \end{tabular}\\\\
      \begin{tabular}{l}
        Integrating position over time:\\
        \end{tabular}
      \end{tabular}
    &
    ...
    \begin{tabular}{l}
      ...
      \end{tabular}\\
      \begin{tabular}{l}
        \greenCode{$f_7($l$,$n$)\,=\,$(car ($f_0$ ($\lambda$ (a x) (cdr a)) ($f_6$ n) l))}\\
        \hspace{\helpSize}($f_7$: \emph{index})
      \end{tabular}
...
Could we also learn the basics of programming, including these foundational sequence manipulation primitives?
...
combined, suggesting a synergy between library learning and
recognition model training. Examining the libraries learned with and without
...
\begin{figure}
  \begin{tabular}{ccc}
    ...
    \begin{tabular}{c}
      ...
      \end{tabular}
  \end{tabular}
  \caption{Test set performance across wake/sleep iterations. Each curve is a run with a different random seed. Teal: Full model. Orange: Dreaming only (no DSL learning). Purple: Consolidation only (no recognition model). Search time plots show solid lines (time averaged over all tasks) and dotted lines (time averaged over solved tasks). Generative text modeling plots show (top) posterior predictive likelihood of held-out strings on held out tasks and (bottom) lower bound on marginal likelihood of held out tasks, both of which are averaged per character.}\label{learningCurves}      
\end{figure}

\begin{figure}
  \includegraphics[width = \textwidth]{figures/dpl/montage.pdf}
  \caption{Top: Learned libraries diagrammed as networks. Arrows point from a learned concept to all other concepts that use it. Bottom: Deeper networks solve more testing tasks, both as measured by average depth (left) and maximum depth (right). Full model (circles on scatterplots) learns deeper libraries than no-recognition-model ablation (crosses on scatterplots). Each scatter plot point run with a different random seed.}\label{depthMontage}
  \end{figure}
...
algorithms.  EC~\cite{Dechter:2013:BLV:2540128.2540316} algorithms alternate between searching, or
`exploring', the space of programs, and then updating their search
procedure by compressing solutions found during exploration. \system
