...
of deep hierarchies of DSL components is characteristic of \system
...
Q_{\text{parent},\text{var},\text{argumentIndex}}(x)/|\text{variables}|&\text{ if }e\in \mathcal{E}
 ...
 \textbf{Symmetry breaking analysis.}
...
\begin{align*}
 ...
 we know that $p^*$ is a member of exactly one equivalence class; call it $\mathcal{E}_*^x$.
 This accomplishes one of our goals:
 we have already shown that $Q^*(p^*|x) = 1$.
 
 Let $q\in \mathcal{E}_i^x\not=\mathcal{E}_*^x$.
 Then because the equivalence classes form a partition we know that
 $q\not= p^*$
 and so $Q^*(q|x) = 0$,
 which is what was to be shown:
 \emph{any}
 program not in $\mathcal{E}_*^x$
