...
Consider an agent tasked with writing programs built from addition, multiplication, and the constants zero and one.
...
that addition should always associate to the right
($Q_{\code{+},\code{+},0} = 0$).
...
\end{align*}
So the $\mathcal{L}^{\text{MAP}}$ prefers $Q_2$ (the symmetry breaking recognition model),
while $\mathcal{L}^{\text{posterior}}$ reverses this preference.
We can confirm this experimentally by training
four recognition models:
a 
