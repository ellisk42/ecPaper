...
\title{\systemEnding: Building Deep Domain Expertise with Wake/Sleep Bayesian Program Learning}
\date{}
\begin{document}
\maketitle{}
...
Here, we present a computational model that takes a step toward the goal of building machines that
grow into domain experts,
looking at a number of problem domains that humans can learn and indeed become experts in.

we draw inspiration from cognitive science.
this inspiration says that there are two things.
we do both these things.
hooray for us.
...
find that,
compared to novices,
experts can, at a glance, categorize and classify problems based on the ``deep structure''
...
our model approaches a specific problem by searching for a program
solving it. It gradually grows its domain-specific knowledge for a
class of problems by assembling a library of code containing concepts
useful for the domain, while jointly training a neural network to
quickly infer, for a specific problem, what code in its library is
likely to solve it. We think of these two learned components as
analogous to the explicit concepts and implicit procedural skill that
human experts develop.  The model, called \systemEnding, is an
instance of what in the machine learning community is called a
wake-sleep algorithm, where for us, `waking' corresponds to solving
problems, while a pair of `sleep' cycles correspond to improving the
library of explicit concepts and training the neural net to improve
...
layers deep, or draws the topmost image in
...
\system works through a novel kind
...
\begin{align}
  p_x &= \argmax_{\substack{p\\Q(p|x)\text{ is large}}} \probability[x|p]\probability[p|\mathcal{D},\theta]\text{, for each task $x\in X$}&&\text{\emph{Wake}}\\
  \mathcal{D},\theta &= \argmax_{\mathcal{D},\theta}\probability\left[\mathcal{D},\theta \right]\prod_{x\in X}\sum_{p\text{ found during waking}} \probability[x|p]\probability[p|\mathcal{D},\theta]&&\text{\emph{Sleep: Abstraction}}\\
  Q(p|x)&\approx \probability[p|x,\mathcal{D},\theta]&&\text{\emph{Sleep: Dreaming}}  
\end{align}
...
%% and
marginalize over refactorings of programs found during waking, seeking to minimize the following expression,
...
%% \end{equation}
\begin{equation}
\underbrace{  -\log \probability[\mathcal{D}]_{\substack{\phantom{p}\\\\\\\\\\}}}_{\text{Description length of library}}\underbrace{-\log \int \probability[\theta|\mathcal{D}]\prod_{x\in X}\sum_{\substack{p \text{ a refactoring of } p'\\ p'\text{ found during waking}}}\probability[x|p]\probability[p|\mathcal{D},\theta]\;\mathrm{d}\theta}_{\text{Description length of tasks given library}}
%% \underbrace{-\log \probability[\mathcal{D}] + \min_{\substack{\theta\\\\\\\\}}\Bigg(-\log
%% \probability[\theta|\mathcal{D}] + \|\theta\|_0}_{\text{Description length of }(\mathcal{D},\theta)} +\sum_{x\in
%%   X}\underbrace{-\log \sum_{\substack{p \text{ a refactoring of } p'\\ p'\text{ found during waking}}}\probability[x|p]\probability[p|\mathcal{D},\theta]}_{\text{Description length of programs for task }x}\Bigg)
\label{AIC}
...
system typically composes around 20 new library routines, and rediscovers
...
agent first learns \code{filter}, then uses
...
Excel~\cite{gulwani2011automating}.  This prior work uses
hand-engineered libraries of primitives and hand-engineered search strategies.  Here, we
will show that we can jointly learn both these ingredients and perform comparably to
...
We tested, but did not train, on the 108 text editing problems from the SyGuS~\cite{alur2016sygus} program synthesis competition. Figure~\ref{textEditExamples} shows representative text-editing problems. Before any learning,
...
\begin{figure}[h]\centering
  \begin{tabular}{ccc}
    ...
    \end{tabular}}
  \end{tabular}
  \caption{Three example text editing tasks. Each task is specified by input/output pairs (notated above as input$\to$output), and the goal is to infer a program producing the input/output mapping.}\label{texEditProblems}
\end{figure}
...
\toprule     \multicolumn{2}{c}{    Subset of 160 tasks}\\\midrule
    \multicolumn{2}{c}{\includegraphics[width = \textwidth]{figures/horizontalLogoTasks.png}}\\\\ 
...
\begin{figure}
  \centering  \begin{tabular}{cc}
    ...
    \multicolumn{2}{c}{\includegraphics[width = \textwidth]{figures/tower_montage_10.png}}\\\midrule \\
    \toprule Example learned library components&Dreams from learned library\\\midrule
    \begin{tabular}{rl}
    ...
    \end{tabular}&
    \begin{tabular}{l}
      \includegraphics[width = 8cm]{figures/tower/dreams/cherry_montage.png}
      \end{tabular}
    \\\bottomrule 
    \end{tabular}
...
physical relationships like the vector form of inverse square laws
...
\begin{figure}
  ...
  \begin{tabular}{lll}\toprule
    ...
    \begin{tabular}{l}
      ...
      \begin{tabular}{l}
        Coulomb's law:\\
        $q_1$, $\vec{r}_1$, $q_2$, $\vec{r}_2\mapsto \frac{q_1q_2}{|\vec{r}_1 - \vec{r}_2|^2} \times \frac{\vec{r}_1 - \vec{r}_2}{|\vec{r}_1 - \vec{r}_2|}$\\
...
In contrast, the correlation between the \emph{number} of learned subroutines and number of solved tasks is weaker ($r=0.20$, $p > 0.05$), although, within each domain, the recognition model still leads to bigger libraries.
These results suggest that the recognition model bootstraps
``better'' libraries, where ``better'' is largely explained by the depth
...
\begin{figure}
  \begin{tabular}{ccc}
    ...
    \begin{tabular}{c}
      ...
      \end{tabular}
  \end{tabular}
  \caption{Test set performance across wake/sleep iterations. Each curve is a run with a different random seed. Teal: Full model. Orange: Dreaming only (no library learning). Purple: Abstraction only (no recognition model). Search time plots show solid lines (time averaged over all tasks) and dotted lines (time averaged over solved tasks). Generative text modeling plots show (top) posterior predictive likelihood of held-out strings on held out tasks and (bottom) lower bound on marginal likelihood of held out tasks, both of which are averaged per character.}\label{learningCurves}      
\end{figure}

\begin{figure}
  \includegraphics[width = \textwidth]{deepMontage.pdf}
  \caption{Top: Learned libraries diagrammed as networks. Arrows point from a learned concept to all other concepts that use it. Bottom: Deeper networks solve more testing tasks, both as measured by average depth (left) and maximum depth (middle). Total library size is more also associated with solving tasks (right), but across domains the association is weaker. Full model (stars on scatterplots) learns bigger and deeper libraries than no-recognition-model ablation (triangles on scatterplots). Each scatter plot point run with a different random seed.}\label{depthMontage}
\end{figure}
...
Our goal was to build a computational model capturing
aspects of learning to be an expert in a new domain of
...
autonomously learn to solve a new domain of problems.
...
with the intention of building agents that develop
...
exploration.
EC : not only one difference. we actually incorporate several improvements.
for example our compression is very sophisticated.
it is a fancy kind of re- factoring.
exploring is analogous to waking except that we also are better here because we can break symmetries and have a deep network.
Both of our sleep phases can be interpreted as a kind of
...
also it turns out that computers need these two too!!!
...
This dynamic is related to, but distinct from, curriculum learning. In curriculum learning approaches an agent solves a stream of pedagogically selected, increasingly difficult tasks, but here we consider the case with the agent lacks a `teacher' and instead self-paces its way through an implicit curriculum.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $\mathcal{B}_x\gets\varnothing $, $\forall x\in X$\Comment{Initialize beams to be empty}
    ...
    \State $\forall x\in \text{batch}$: $\mathcal{B}_x\gets\mathcal{B}_x\cup \left\{p\;|\;p\in \text{enumerate}(\probability[\cdot |\mathcal{D},\theta],T)\text{ if }\probability[x|p] > 0 \right\}$\Comment{Wake}
    \State Train $Q(\cdot |\cdot )$ to minimize $\mathcal{L}^{\text{MAP}}$ across all $\left\{\mathcal{B}_x \right\}_{x\in X}$\Comment{Dream Sleep}
    \State $\forall x\in \text{batch}$: $\mathcal{B}_x\gets\mathcal{B}_x\cup \left\{p\;|\;p\in \text{enumerate}(Q(\cdot |x),T)\text{ if }\probability[x|p] > 0 \right\}$\Comment{Wake}
    \State $\forall x\in \text{batch}$: $\mathcal{B}_x\gets\text{ top }F\text{ elements of }\mathcal{B}_x\text{ as measured by }\probability[\cdot |x,\mathcal{D},\theta]$\Comment{Keep top $F$ programs}
    \State $\mathcal{D},\theta,\left\{\mathcal{B}_x \right\}_{x\in X}\gets $\textsc{Consolidate}$(\mathcal{D},\theta,\left\{\mathcal{B}_x \right\}_{x\in X})$\Comment{Consolidation Sleep}
    \State \textbf{yield} $(\mathcal{D},\theta)$, $Q$, $\left\{\mathcal{B}_x \right\}_{x\in X}$ \Comment{Yield the updated DSL, recognition model, and solutions found to tasks}
    \EndWhile
    \EndWhile
...
\begin{algorithm}%[H]
  ...
  \caption{Library Induction Algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{Output:} Library $\mathcal{D}$, weight vector $\theta$
    ...
    \State Define $L(\mathcal{D}',\theta) =  \prod_x \sum_{p\in \mathcal{B}_x} \probability[x|p]\probability[\text{\textsc{refactor}}(v_p|\mathcal{D}')|\mathcal{D}',\theta]$ \Comment{Likelihood under $(\mathcal{D}',\theta)$}
    ...
    \State components $\gets$ $\left\{\textsc{refactor}(v|\mathcal{D})\;:\;\forall x, \forall p\in \mathcal{B}_x, \forall v\in \text{children}(v_p) \right\}$ \Comment{Propose many new components}
    \State proposals $\gets$ $\left\{\mathcal{D}\cup\left\{c \right\}\;:\;\forall c\in \text{components} \right\}$ \Comment{Propose many new libraries}
    \State $\mathcal{D}'\gets \argmax_{\mathcal{D}'\in \text{proposals}}\text{score}(\mathcal{D}') $\Comment{Get highest scoring new library}
    \State \textbf{if }$\text{score}(\mathcal{D}') < \text{score}(\mathcal{D})$\textbf{ return }$\mathcal{D},\theta^*(\mathcal{D})$\Comment{No changes to library led to a better score}
    \State $\mathcal{D}\gets\mathcal{D}'$ \Comment{Found better library. Update library.}
    \State $\forall x\;:\;\mathcal{B}_x\gets\left\{\text{\textsc{refactor}}(v_p|\mathcal{D})\;:\; p\in \mathcal{B}_x\right\}$\Comment{Refactor beams in terms of new library}
    \EndWhile
  \end{algorithmic}
\end{algorithm}
...
members of a version space's extension in terms of a new library.
...
\begin{align*}
  ...
  \end{align*}
where $\text{size}(e|\mathcal{D})$ for program $e$ and library $\mathcal{D}$
...
We now formally prove that $I\beta_n$ exhaustively enumerates the space of possible refactorings.
...
Having introduced the version-space machinery, we now are in a position to define
how these version spaces aggregate into a single data structure, one for each program, tracking every equivalence revealed by
$I\beta_n$.
Observe that every time we calculate $I\beta_n(p)$, we obtain a version space containing expressions semantically equivalent to program $p$.
In line with prior work in program analysis~\cite{tate2009equality},
we track these equivalences,
and finally return a single structure for each program compiling all of these equivalences.
Concretely,
Algorithm~\ref{grammarInductionAlgorithm} calculates (line 5), for each program $p$,
a version space $I\beta(p)$ defined as
\begin{equation*}
  I\beta(p) = I\beta_n(p)\uplus\begin{cases}
  \text{if $p = (f\;x)$: }I\beta(f)\;I\beta(x)\\
  \text{if $p = \lambda b$: }\lambda I\beta(b)\\
  \text{if $p$ is an index or primitive: }\varnothing 
  \end{cases}
\end{equation*}
where $n$, the amount of refactoring, is a hyper parameter. We set $n$ to 3 for all experiments with the exception of learning recursive functions from basic Lisp primitives (Appendix~\ref{appendixMcCarthy}),
where more involved refactoring was required; for that experiment we set $n = 4$.
...
\subsubsection{Computational complexity of library learning}

How long does each update to the library in
...
program (line 6), so this means we spend $O(V^2) = O(P^22^n)$ per library
...
function from a library $\mathcal{D}$ to a shortest program in
...
and $\mathcal{D}$ is the current library:
\begin{align*}
...
we take only the top $I$ proposals as measured by $-\sum_x\min_{p\in \mathcal{B}_x}\text{beam}_{v_p}(\mathcal{D}')$.
...
Next,
we describe our MAP estimator for the continuous parameters ($\theta$) of the generative model.



\subsubsection{Estimating the continuous parameters $\theta$ of the generative model}\label{mapAppendix}
We use an EM algorithm to estimate the continuous parameters of the generative model, i.e. $\theta$.
...
\label{maximizeStep}  \theta& = \argmax_\theta \log P(\theta) + \sum_x \expect_{q_x}\left[\log \probability\left[p|\theta \right] \right]\\
  q_x(p)&\propto \probability[x|p]\probability[p|\theta]\indicator\left[p\in \mathcal{B}_x \right]
\end{align}
...
of the types of the expressions in the library are not polymorphic, in which case our library is equivalent to a PCFG
and this estimator is equivalent to the inside/outside algorithm.
Polymorphism introduces context-sensitivity to the library,
...
the current library, running them to get their outputs,
...
finding the MAP program solving a task drawn from the library.  To make
...
where $A$ is the maximum arity\footnote{The arity of a function is the number of arguments that it takes as input.} of any primitive in the library.
...
specifies a procedure for drawing samples from $Q(\cdot |X)$.
 \begin{algorithm}
   ...
   \begin{algorithmic}[1]
     ...
     \State {\bfseries Input:} recognition model $Q$, task $x$, library $\mathcal{D}$, type $\tau$
     \State \textbf{Output:} a program whose type unifies with $\tau$
     ...
     \State {\bfseries Input:} recognition model $Q$, task $x$, library $\mathcal{D}$, parent $\in \mathcal{D}\cup\left\{\text{start},\text{var} \right\}$, argumentIndex $\in \mathbb{N}$, environment $\mathcal{E}$, type $\tau$
     \State \textbf{Output:} a program whose type unifies with $\tau$
     \If{$\tau = \alpha\to\beta$}\Comment{Function type --- start with a lambda}
     \State var $\gets$ an unused variable name
...
and two programs in beam $\mathcal{B}_x$,
...
To be clear,
our recognition model does not learn to break \emph{every} possible symmetry in every possible library or DSL.
...
Figure~\ref{allTSNE} shows how \system organizes and represents its tasks over its learning trajectory for several domains.
We visualize the inferred program representations (left columns) by
calculating a feature vector for each task $x$, written $\phi(x)$,
whose $i^{\text{th}}$ component is the expected number of times that the $i^\text{th}$  library component was used when solving the task:
\[
\phi(x)_i = \expect_{\probability[p |\mathcal{D},\theta,x]}\left[ \text{number of times }i\in \mathcal{D} \text{ occurs in }p\right]
\]
and then embed those feature vectors in a two dimensional space using tSNE~\cite{maaten2008visualizing}.
We visualize the recognition model's task representations by
flattening the neural output $Q_{ijk}(x)$ (Appendix~\ref{recognitionAppendix}) to
obtain a feature vector for each task, which we visualize also using tSNE both after the first wake/sleep cycle (left column) and after the final wake/sleep cycle (middle column).
\begin{figure}
  \includegraphics[width = \textwidth]{figures/all_tsne.png}
  \caption{Left: TSNE embedding of model's `at-a-glance' (before searching for solution) organization of tasks (neural net activations), after first wake/sleep cycle. Middle: at-a-glance organization after last wake/sleep cycle. Right: TSNE visualization of programs actually used to solve tasks; intuitively, how model organizes tasks after searching for a solution. We convert each program into a feature vector by counting the \# times each library routine is used, and then embed those feature vectors.}\label{allTSNE}
\end{figure}
...
\subsection{Learning a language for physical laws}\label{physicsAppendix}

We took equations from MCAT as well as AP Physics B \& C cheat sheets (\url{http://mcat.prep101.com/wp-content/uploads/ES_MCATPhysics.pdf}; \url{https://secure-media.collegeboard.org/digitalServices/pdf/ap/ap-physics-1-equations-table.pdf}; \url{https://secure-media.collegeboard.org/digitalServices/pdf/ap/physics-c-tables-and-equations-list.pdf}),
choosing representative laws,
but excluding expressions with trigonometric functions, multivariate differentials, and integrals.
These equations included  free fall velocity ($\sqrt{2gh}$),
velocity under ballistic motion ($v^2 = v_0^2 + 2a(x - x_0)$),
velocity magnitude ($v = \sqrt{v_x^2 + v_y^2}$),
angular acceleration ($a_r = v^2/R$),
mass energy equivalence ($e = mc^2$),
expressions for center of mass ($\sum_i m_ix_i/\sum_im_i$ and $(m_1\vec{x_1} + m_2\vec{x_2})/(m_1 + m_2)$),
density ($\pho = m/v$), pressure ($F/A$), power ($P = I^2R$ and $P = V^2/R$),
RMS voltage ($V/\sqrt{2}$), energy stored in a capacitor ($U = 1/2CV^2$, $1/2QV$, and $1/2Q^2/C$),
optical power ($P = 1/f$),

Each equation was converted to a symbolic regression problem by sampling 10 random input/output examples,
where the inputs correspond to free variables in the target expression.
Physical constants were expressed in Planck units, which generally drops certain special coefficients from
these expressions (e.g., the gravitational constant $G = 1$).
We gave the system the following primitives: \code{0}, \code{1}, $\pi$, \code{power}, \code{+}, \code{*}, \code{/}, \code{-}, \code{map}, \code{fold}, \code{zip}, \code{cons}, \code{car}, \code{cdr}, and \code{nil}.

At each wake cycle, rather than sample a random minibatch of tasks,
we gave the agent all tasks it had not solved in the past two wake cycles.
This had the effect of the agent solving the easiest tasks first,
and then reallocating its compute time to a smaller set of harder, unsolved tasks.



\subsection{Learning a language of recursive functions}\label{appendixMcCarthy}
...
\begin{center}
  \begin{tabular}{ccccccc}
    ...
    Physical laws&4h&64&See~\ref{physicsAppendix}&1 & 30&5\\
    Recursive functions&4h&64&See~\ref{appendixMcCarthy}&1 & 30&5\\
    \bottomrule     
  \end{tabular}
\end{center}
