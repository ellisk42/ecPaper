...
%    \draw (0,0) rectangle (10,5);
    \node(t1)[align=left] at (1,-1) {
      \small +106 769-43$\to $106.769.43\\
      \small +83 973-831$\to $83.973.831\\
      \small $f(\text{\code{s}}) = $\code{(}\subnode{f01}{$f_0$}\code{  \codechar{.} \codechar{-}}\\
      \small \hspace{1.25cm}\code{($f_0$ \codechar{.} \codechar{ }}\\
      \small \hspace{1.4cm}\code{(cdr s)))}
    };
    \node(t2)[align=left] at ([xshift=2cm]t1.east) {
      \small Dream Coder$\to$Dr. Dream\\
      \small Andy Cencici$\to$Dr. Andy\\
      \small Jan Kotas$\to$Dr. Jan\\
      \small Nancy Jones$\to$Dr. Nancy\\
      $f(\text{\code{s}}) = $\code{($f_3$ ($f_4$ \codechar{ } s))}
    };
    \node(t3)[align=left] at ([xshift=2cm]t2.east) {
      \small Dream (Coder)$\to$Coder\\
      \small 1 (2) 3 4$\to$2\\
      \small the (dog) ran$\to$ran\\
      \small +817(33)99$\to$33\\
      $f(\text{\code{s}}) = $\code{($f_6$ \codechar{(} \codechar{)} s)}
    };

    \draw (-2,-2.5) -- (15,-2.5);
    \node[align=left] at (-1.5,-2.3) {Tasks and\\\texttt{Programs}\\\\DSL};

    \node(substitution)[draw,rounded corners,align=center] at (1,-4) {
      \small $f_0($\code{s}$,$\code{a}$,$\code{b}$) \,=\, $\\
       \mcode{(map ($\lambda$ (x) (if}\\
      \mcode{(= x a) b x)) s)}
    };
    \node[anchor = north] at (substitution.south) {substitute characters};
    \node(cat)[draw,rounded corners,align=center] at (1,-6) {
      \small $f_1($\code{a}$,$\code{b}$) \,=\, $\\
       \mcode{(fold ($\lambda$ (x y)}\\\mcode{ (cons x y)) a b))}
    };
    \node[anchor = north] at (cat.south) {concat strings};
    \node(looping)[draw,rounded corners,align=center] at (1,-8) {
      \small $f_2($\code{c}$,$\code{p}$,$\code{s}$) \,=\, $\\
       \mcode{(unfold l ($\lambda$ (x)}\\\mcode{(= c (car x)))}\\\mcode{ p ($\lambda$ (z) (cdr z)))}
    };
    \node[anchor = north] at (looping.south) {loop until char};

    \node(constant)[draw,rounded corners,align=center] at (4.5,-4) {
      \small $f_3($\code{s}$) \,=\, $\\
       \mcode{($f_0$ s string)}
    };
    \node[anchor = north] at (constant.south) {prepend constant};
    \node(drop)[draw,rounded corners,align=center] at (4.5,-6) {
      \small $f_4($\code{c}$) \,=\, $\\
      \mcode{($f_2$ c ($\lambda$ }\\\mcode{(z) (car z)))}
    };
    \node[anchor = north] at (drop.south) {prefix};
    \node(suffix)[draw,rounded corners,align=center] at ([yshift=-2cm]drop.center) {
      \small $f_5($\code{s}$,$\code{c}$) \,=\, $\\
      \mcode{(fold ($\lambda$ (x y) }\\\mcode{(cdr (if (= x c)}\\\mcode{ s y))) s s)}
    };
    \node[anchor = north] at (suffix.south) {suffix};
    \node(delimited)[draw, rounded corners, align=center,text width=2.5cm] at ([xshift=3.5cm]constant.center) {
      \code{$f_6($x$,$y$,$s$)\,=\,$}\\
      \mcode{($f_4$ x ($f_5$ y c))}};
    \node[anchor = north] at (delimited.south) {delimited substring};
    \draw[->] (delimited.west) -- (drop.east);
    \draw[->] (delimited.west) -- (suffix.east);

    \node(appendSingle)[draw, rounded corners, align=center,text width=2.5cm] at ([yshift=-2cm]delimited.center) {
      \code{$f_7($c$,$s$)\,=\,$}\\
      \mcode{($f_1$ (cons c empty) s)}};
    \node[anchor = north] at (appendSingle.south) {append character};
    
    \node(surround)[draw, rounded corners, align=center,text width=2.5cm] at ([yshift=-2cm]appendSingle.center) {
      \code{$f_8($s$)\,=\,$}\\
      \mcode{(cons \codechar{(} ($f_7$ \codechar{)} s))}};
    \node[anchor = north] at (surround.south) {surround in parentheses};
    ...
    \draw[->] ([yshift=0.5cm,xshift=-0.4cm]t1.south) to[in=100,out=200] (substitution.north);
    \draw[->] ([yshift=0.8cm,xshift=-0.5cm]t1.south) to[in=120,out=200] ([xshift=-0.5cm]substitution.north);
