...
is the standard notation for ``expression $p'$ evaluates to $p$ by the standard rules of $\lambda$-calculus''~\cite{pierce}.
...
function of the size of the program being refactored.  Next, we show
how to tame this exponential explosion.
...
\begin{itemize}
\item A deBuijn index: written $\$i$, where $i$ is a natural number
\item An abstraction: written $\lambda v$, where $v$ is a version space
\item A  application: written $(f \;x)$, where both  $f$ and $x$ are version spaces
...
\begin{align*}
  \textsc{refactor}(v|\mathcal{D}) &= \begin{cases}
    e\text{, if $e\in \mathcal{D}$ and $e\in v$}\\
  ...
  \textsc{refactor}'(\uplus V|\mathcal{D}) &= \argmin_{e\in \left\{\textsc{refactor}(v|\mathcal{D})\;:\;v\in V \right\}}\ell(e|\mathcal{D})
  \end{align*}
where $\ell(e|\mathcal{D})$ for program $e$ and DSL $\mathcal{D}$
is the size of the syntax tree of $e$,
when members of $\mathcal{D}$ are counted as having size 1.
Concretely, $  \textsc{refactor}(v|\mathcal{D})$ calculates $\argmin_{p\in \denotation{v}}\ell(p|\mathcal{D})$.
...
%         \STATE \textbf{Hyperparameters:} Pseudocounts $\alpha$, regularization parameter $\lambda$
         ...
         \STATE Define $\text{refactor}(p|\mathcal{D}') = \argmin_{e\in \denotation{v_p}} \ell (e|\mathcal{D}')$ \Comment{Rewrites $p$ assuming $\mathcal{D}'$ is the DSL}
...
Observe that every $\lambda$-calculus expression is also a version space,
and that, if $e$ is an expression, then $\denotation{e} = \left\{e \right\}$.

\begin{align}
  S_n(v)& = \left\{\shift{n}v\mapsto \$n \right\}\cup
  \begin{cases}
    \text{if $v$ is primitive:}&\left\{\Lambda\mapsto v \right\}\\
    \text{if $v = \$i$ and $i < n$:}&\left\{\Lambda\mapsto \$i \right\}\\
    \text{if $v = \$i$ and $i\geq n$:}&\left\{\Lambda\mapsto \$(i + 1) \right\}\\
    \text{if $v = \lambda b$:}&\left\{v'\mapsto \lambda b' \;:\; v'\mapsto b'\in S_{n + 1}(b)  \right\}\\
    \text{if $v = (f\;x)$:}&\left\{v_1\cap v_2\mapsto (f'\;x') \;:\; v_1\mapsto f'\in S_n(f),\; v_2\mapsto x'\in S_n(x) \right\}\\
