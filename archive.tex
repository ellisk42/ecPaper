...
\begin{figure}
  \begin{tikzpicture}[line width=0.4mm]
    ...
    \node at (0.5,-4.75) {\textbf{\textsc{Memory Consolidation}}};
    \draw (-2,-5) -- (3,-5) -- (3,-10) -- (5.5,-10) -- (5.5,-13) -- (-2,-13) -- (-2,-5);
    \node at (11.5,-4.75) {\textbf{\textsc{Dreaming}}};
    ...
    \begin{scope}[shift={(9,-4.5)}]
      ...
      \draw [dashed,red] (p1.south) to[out=-120,in=180] node[above,fill=white]{\color{black}Loss} ([xshift=-0.4cm]n.west);
    ...
    \begin{scope}[shift={(-2,-4.5)}]
      \node[align=center] at (0.8,-1.2) (f1){progs. for\\task$_1$:\\\scode{(+ (car z) 1)}};
      \node[align=center] at ([xshift = 1cm]f1.east) (f2){progs. for\\task$_2$:\\\scode{(cons (+ 1 1))}};
      \node[align=center] at ([yshift = 0cm,xshift=1cm]f2.east) (f3){\textbf{DSL}\\\scode{cons}, \scode{+}, \\...};
      \node(c)[rectangle, rounded corners, dashed, draw, minimum width = 3.75cm, minimum height = 3cm, anchor = north] at ([yshift=-0.8cm]f2.south) {};
...
This dream-learning architecture unifies two lines of prior work.
One line of work
...
\system~takes as its goal to
acquire domain-specific expertise,
which it learns by
solving a collection of
programming \textbf{tasks} -- typically about a hundred.
During its Wake phase,
it searches for programs solving tasks,
guided by its current DSL and recognition model.
It cycles through Wake
