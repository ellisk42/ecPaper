...
problems adressed with our method. The original DSL, while inspired by the
Logo~\cite{abelson1974logo} in the sense that ones controls a moving on-off
point, was designed to be congitively plausible in the sense that some
abstractions are baked in for curves and a few structures --- it was first
described in~\cite{sablemeyer2017geom}.
...
In this context programs to be learned are directly resulting shapes, since they
don't take an input, and all tasks will be unique in the sense that each program
ignores input and produces a single output.
...
\begin{wrapfigure}{r}{0.25\textwidth}
  ...
  \end{center}\label{dslgeom}\caption{Some defaults for \texttt{Integrate}}
\end{wrapfigure}
...
The training was done on the following set of tasks
