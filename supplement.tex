...
We use Algorithm~\ref{grammarInductionAlgorithm} to search for the structure of the DSL that best explains the frontiers.
...
We write $c(e,p)$ to mean the number of times that primitive $e$ was used in program $p$;  $R(p)$ to mean the sequence of types input to sample in Alg.~1 of the main paper. Jensen's inequality gives an intuitive lower bound on the likelihood of a program $p$:
\begin{align*}
...
When is this lower bound tight? This lower bound is tight whenever all
...
enable the learning of the \texttt{filter}, \texttt{index}, and
\texttt{slice} concepts. Greek letters represent polymorphic types.
...
using importance sampling (so the expectation in~\ref{EM} is taken
over $q$ and we reweight using $Q_x$) or by directly using $q$ as our approximate posterior over the program that solves task $x$.
...
and \emph{not} be a function of how the previous generative model weighted them.
In practice,
